...
int create_helper(int event_fd, int err_fd, uid_t uid, gid_t gid, long max_fd)
{
  ...
  if (pipe(pipefd) == -1 || !fix_fd(pipefd[1]) || (pid = fork()) == -1)
    {
      send_event(err_fd, EVENT_PIPE_ERR, errno, nullptr);
      _exit(0);
    }
  ...
  while(1)
    {
      ...
      char *p, *action_str, *hostname = nullptr, *domain = nullptr;
      unsigned char *buf = (unsigned char *)daemon->namebuff;
      unsigned char *end, *extradata, *alloc_buff = nullptr;
      int is6, err = 0;
      int pipeout[2];
	  ...
	  while (1)
	      ...
	      if (rc == pid)
		{
		  ...
		  if (WIFSIGNALED(status))
		    send_event(event_fd, EVENT_KILLED, WTERMSIG(status), nullptr);
		  else if (WIFEXITED(status) && WEXITSTATUS(status) != 0)
		    send_event(event_fd, EVENT_EXITED, WEXITSTATUS(status), nullptr);
		  break;
		}
      ...
      if (data.action != ACTION_TFTP && data.action != ACTION_ARP)
	{
	  ...
	  if (is6)
	    ...
	    my_setenv("DNSMASQ_RELAY_ADDRESS", data.giaddr.s_addr != 0 ? inet_ntoa(data.giaddr) : nullptr, &err);
	  ...
	  my_setenv("DNSMASQ_TIME_REMAINING", data.action != ACTION_DEL && data.remaining_time != 0 ? daemon->dhcp_buff2 : nullptr, &err);
	  
	  my_setenv("DNSMASQ_OLD_HOSTNAME", data.action == ACTION_OLD_HOSTNAME ? hostname : nullptr, &err);
	  if (data.action == ACTION_OLD_HOSTNAME)
	    hostname = nullptr;
	  
	  my_setenv("DNSMASQ_LOG_DHCP", option_bool(OPT_LOG_OPTS) ? "1" : nullptr, &err);
	}
      ...
      if (err == 0)
	{
	  execl(daemon->lease_change_command, 
		...
		daemon->addrbuff, hostname, (char*)nullptr);
	  err = errno;
	}
      /* failed, send event so the main process logs the problem */
      send_event(event_fd, EVENT_EXEC_ERR, err, nullptr);
      _exit(0); 
    }
}
