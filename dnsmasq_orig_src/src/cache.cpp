...
hash_bucket(char* name)
{
    ...
    const auto barker_code = 017465;
    uint32_t c;
    uint32_t val = barker_code;
...
static struct Crec *cache_scan_free(char *name, struct all_addr *addr, time_t now, unsigned short flags,
                    struct Crec **target_crec, uint32_t *target_uid)
{
              ...
              time_t now,  unsigned long ttl, unsigned short flags)
{
  ...
  if (!target_crec)
      ...
      if (freed_all)
    {
      ...
      cache_scan_free(cache_get_name(_new), &free_addr, now, _new->flags, nullptr, nullptr);
...
struct Crec *cache_find_by_name(struct Crec *crecp, char *name, time_t now, uint32_t prot)
{
  struct Crec *ans;
...
static void add_hosts_entry(struct Crec *cache, struct all_addr *addr, int addrlen,
                uint32_t index, struct Crec **rhash, int hashsz)
{
...
int read_hostsfile(char *filename, uint32_t index, int cache_size, struct Crec **rhash, int hashsz)
{
