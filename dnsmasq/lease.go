...
static struct dhcp_lease *leases = nullptr, *old_leases = nullptr;
...
# error Buffer size breakage in leasefile parsing.
	...
	*daemon->namebuff = *daemon->dhcp_buff = *daemon->packet = '\0';
	...
	if (strcmp(daemon->dhcp_buff3, "duid") == 0)
	  {
	    daemon->duid_len = parse_hex(daemon->dhcp_buff2, (unsigned char *)daemon->dhcp_buff2, 130, nullptr, nullptr);
	...
	if (inet_pton(AF_INET, daemon->namebuff, &addr.addr.addr4))
	  {
	    ...
	    hw_len = parse_hex(daemon->dhcp_buff2, (unsigned char *)daemon->dhcp_buff2, DHCP_CHADDR_MAX, nullptr, &hw_type);
	...
	else if (inet_pton(AF_INET6, daemon->namebuff, &addr.addr.addr6))
	  {
	    ...
	    if ((lease = lease6_allocate(&addr.addr.addr6, lease_type)))
	      {
		lease_set_iaid(lease, strtoul(s, nullptr, 10));
	...
	if (!lease)
	  die (_("too many stored leases"), nullptr, EC_MISC);

	if (strcmp(daemon->packet, "*") != 0)
	  clid_len = parse_hex(daemon->packet, (unsigned char *)daemon->packet, 255, nullptr, nullptr);
	
	lease_set_hwaddr(lease, (unsigned char *)daemon->dhcp_buff2, (unsigned char *)daemon->packet, 
			 hw_len, hw_type, clid_len, now, 0);
	
	if (strcmp(daemon->dhcp_buff, "*") !=  0)
	  lease_set_hostname(lease, daemon->dhcp_buff, 0, domain, nullptr);
...
void lease_update_from_configs(void)
{
  ...
  for (lease = leases; lease; lease = lease->next)
    if (lease->flags & (LEASE_TA | LEASE_NA))
      continue;
    else if ((config = find_config(daemon->dhcp_conf, nullptr, lease->clid, lease->clid_len,
				   lease->hwaddr, lease->hwaddr_len, lease->hwaddr_type, nullptr)) &&
	     (config->flags & CONFIG_NAME) &&
	     (!(config->flags & CONFIG_ADDR) || config->addr.s_addr == lease->addr.s_addr))
      lease_set_hostname(lease, config->hostname, 1, get_domain(lease->addr), nullptr);
    else if ((name = host_from_dns(lease->addr)))
      lease_set_hostname(lease, name, 1, get_domain(lease->addr), nullptr); /* updates auth flag only */
}
...
static void kill_name(struct dhcp_lease *lease)
{
  ...
  lease->hostname = lease->fqdn = nullptr;
}

void lease_set_hostname(struct dhcp_lease *lease, const char *name, int auth, char *domain, char *config_domain)
{
  struct dhcp_lease *lease_tmp;
  char *new_name = nullptr, *new_fqdn = nullptr;
