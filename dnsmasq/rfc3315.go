...
static int dhcp6_no_relay(struct state *state, int msg_type, void *inbuff, size_t sz, int is_unicast, time_t now)
{
  ...
  struct dhcp_config *config = nullptr;
  ...
  if (state->clid)
    {
      config = find_config(daemon->dhcp_conf, state->context, state->clid, state->clid_len, state->mac, state->mac_len, state->mac_type, nullptr);
      ...
      else if (state->client_hostname)
	{
	  ...
	  if (strlen(state->client_hostname) != 0)
	      ...
	      if (!config)
		{
		  ...
		  struct dhcp_config *new = find_config(daemon->dhcp_conf, state->context, nullptr, 0, nullptr, 0, 0, state->hostname);
  ...
  if (config)
	...
	ignore = 1;
	   ...
	   find_config(daemon->dhcp_conf, nullptr, state->clid, state->clid_len, state->mac, state->mac_len, state->mac_type, nullptr))
    {
  ...
  switch (msg_type)
	...
	*outmsgtypep = DHCP6ADVERTISE;
	...
	if (opt6_find(state->packet_options, state->end, OPTION6_RAPID_COMMIT, 0))
  	...
  	log6_quiet(state, "DHCPSOLICIT", nullptr, ignore ? _("ignored") : nullptr);
	  ...
	  {   
		...
		if (dump_all_prefix_classes)
		  ...
		  { 
		    ...
		    if (p && option_bool(OPT_LOG_OPTS))
		      my_syslog(MS_DHCP | LOG_INFO, "%u prefix class %d tag:%s", state->xid, p->_class, p->tag.net);
		  }
	      }
#endif
		  ...
		  check_address(state, &addr))
		{
		  ...
		  if (dump_all_prefix_classes && state->ia_type == OPTION6_IA_NA)
		    state->send_prefix_class = prefix_class_from_context(c);
#endif
		  add_address(state, c, lease_time, nullptr, &min_time, &addr, now);
		...
		if ((c = address6_available(state->context, &req_addr, solicit_tags, plain_range)))
		  {
		    ...
		    if (dump_all_prefix_classes && state->ia_type == OPTION6_IA_NA)
		      state->send_prefix_class = prefix_class_from_context(c);
#endif
		    add_address(state, c, c->lease_time, nullptr, &min_time, &req_addr, now);
		    mark_context_used(state, &req_addr);
		    get_context_tag(state, c);
		    address_assigned = 1;
		  }
		...
		if (dump_all_prefix_classes && state->ia_type == OPTION6_IA_NA)
		  state->send_prefix_class = prefix_class_from_context(c);
#endif
		add_address(state, c, c->lease_time, nullptr, &min_time, &addr, now);
		mark_context_used(state, &addr);
		get_context_tag(state, c);
		address_assigned = 1;
	      }
	...
	if (address_assigned) 
	  ...
	  { 
		...
		{
		  log6_packet(state, state->lease_allocate ? "DHCPREPLY" : "DHCPADVERTISE", nullptr, _("no addresses available"));
		  break;
		}
	  }
	...
	*outmsgtypep = DHCP6REPLY;
	...
	log6_quiet(state, "DHCPREQUEST", nullptr, ignore ? _("ignored") : nullptr);
	...
	if (address_assigned) 
	  ...
	  { 
	    ...
	    put_opt6_string(_("no addresses available"));
	    end_opt6(o1);
	    log6_packet(state, "DHCPREPLY", nullptr, _("no addresses available"));
	  }
	...
	*outmsgtypep = DHCP6REPLY;
	...
	log6_quiet(state, "DHCPRENEW", nullptr, nullptr);
	...
	for (opt = state->packet_options; opt; opt = opt6_next(opt, state->end))
	  {
	    ...
	    for (; ia_option; ia_option = opt6_find(opt6_next(ia_option, ia_end), ia_end, OPTION6_IAADDR, 24))
	      {
		struct dhcp_lease *lease = nullptr;
		struct in6_addr req_addr;
	...
	*outmsgtypep = DHCP6REPLY;
	...
	log6_quiet(state, "DHCPCONFIRM", nullptr, nullptr);
	...
	if (state->context && state->context->netid.net && !state->context->current)
	  {
	    state->context->netid.next = nullptr;
	...
	if (state->hostname_auth)
	  ...
	  state->send_domain = get_domain6(nullptr);

	log6_quiet(state, "DHCPINFORMATION-REQUEST", nullptr, ignore ? _("ignored") : state->hostname);
	if (ignore)
	  return 0;
	*outmsgtypep = DHCP6REPLY;
	...
	*outmsgtypep = DHCP6REPLY;
	...
	log6_quiet(state, "DHCPRELEASE", nullptr, nullptr);
	...
	*outmsgtypep = DHCP6REPLY;
	...
	log6_quiet(state, "DHCPDECLINE", nullptr, nullptr);
...
static struct dhcp_netid *add_options(struct state *state, int do_refresh)  
{
  ...
  if (state->hostname)
	...
	{
	  *(p++) = state->fqdn_flags;
	  p = do_rfc1035_name(p, state->hostname, nullptr);
	  if (state->send_domain)
	    {
	      p = do_rfc1035_name(p, state->send_domain, nullptr);
	      *p = 0;
	    }
	}
      end_opt6(o);
  ...
  if (option_bool(OPT_LOG_OPTS) && oro)
    {
      ...
      for (i = 0; i <  opt6_len(oro) - 1; i += 2)
	{
	  char *s = option_string(AF_INET6, opt6_uint(oro, i, 2), nullptr, 0, nullptr, 0);
  ...
  if (state->send_prefix_class)
    {
      ...
      put_opt6_short(state->send_prefix_class->_class);
      end_opt6(o1);
    }
#endif
