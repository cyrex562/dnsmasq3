...
void rand_init()
{
  ...
  if (fd == -1 ||
    ...
    die(_("failed to seed the random number generator: %s"), nullptr, EC_MISC);
...
void safe_pipe(int *fd, int read_noblock)
{
  if (pipe(fd) == -1 || 
    ...
    die(_("cannot create pipe: %s"), nullptr, EC_MISC);
}
		  ...
		  for (j = 0; j < bytes; j++)
		      ...
		      if (j < bytes - 1)
			{
			 ...
			 is illegal. */
		      if (strchr(&in[j*2], '*'))
			return -1;
		      out[i] = strtol(&in[j*2], nullptr, 16);
		      mask = mask << 1;
		      if (++i == maxlen)
			break; 
		      if (j < bytes - 1)
