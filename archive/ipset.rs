...
pub fn ipset_init() {
    ...
    die("failed to create IPset control socket: %s"       *const libc::c_char , 0 ,
        5);
}
fn new_add_to_ipset(mut setname: *const libc::c_char,
                                      ...
                                      -> i32 {
    let mut nlh: nlmsghdr = 0 );
    let mut nfg: my_nfgenmsg = 0 );
    let mut nested: [my_nlattr; 2] = [0 ; 2];
    let mut proto: u8 = 0;
    let mut addrsz: i32 =
        if af == 10 {
            16
...
pub fn add_to_ipset(mut setname: *const libc::c_char,
    ...
    if ret == -(1) {
        my_syslog(3,
                  "failed to update ipset %s: %s", setname,
