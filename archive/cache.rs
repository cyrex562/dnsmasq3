...
use crate::defines::{
    BigName, C2rustUnnamed10, C2rustUnnamed8, Cname, Crec, DnsmasqDaemon, HostRecord, HostsFile,
    InterfaceName, MxSrvRecord, NaPtr, NameListEntry, NetAddress, PtrRecord, Server, TxtRecord,
    TYPESTR, _ISPRINT, _ISSPACE,
};
use crate::domain::{get_domain, get_domain6};
use crate::inotify::set_dynamic_inotify;
use crate::util::{canonicalise, hostname_isequal, prettyprint_addr, NetAddress_isequal};
use crate::{
    blockdata::{
        blockdata_free, blockdata_read, blockdata_report, blockdata_retrieve, blockdata_write,
    },
    util::inet_ntop,
};
...
fn rehash(mut size: i32) {
    ...
    } else if new_size <= hash_size || { false } {
        return;
    }
...
fn hash_bucket(hash_table: &mut Vec<Crec>, mut name: &String) -> Crec {
    let mut c: u32 = 0;
    let mut val: u32 = 0o17465;
    let mut mix_tab = String::new();
    loop {
        let fresh7 = name;
        name = name.offset(1);
        c = *fresh7;
    ...
    /* hash_size is a power of two */
    return hash_table.offset(((val ^ val >> 16) & (hash_size - 1)));
}
fn cache_hash(mut crecp: Crec) {
    ...
    let mut up: Crec = hash_bucket(cache_get_name(crecp)); /* invalidate CNAMES pointing to this. */
    if crecp.flags & (1) << 2 == 0 {
        while !up.is_null() && (**up).flags & (1) << 2 != 0 {
            up = &mut (**up).hash_next
        }
        if crecp.flags & (1) << 0 != 0 {
            while !up.is_null() && (**up).flags & (1) << 0 == 0 {
                up = &mut (**up).hash_next
            }
        }
    }
...
pub fn cache_get_name(mut crecp: Crec) -> String {
    if crecp.flags & (1) << 9 != 0 {
...
pub fn cache_get_cname_target(mut crecp: Crec) -> String {
...
fn is_outdated_cname_pointer(mut crecp: Crec) -> i32 {
    if crecp.flags & (1) << 11 == 0 || crecp.addr.cname.is_name_ptr != 0 {
        return 0;
    }
    /* NB. record may be reused as DS or DNSKEY, where uid is
    overloaded for something completely different */
    if !crecp.addr.cname.target.cache.is_null()
        && (*crecp.addr.cname.target.cache).flags & ((1) << 12 | (1) << 14) == 0
...
fn is_expired(mut now: time::Instant, mut crecp: Crec) -> i32 {
    if crecp.flags & (1) << 0 != 0 {
        return 0;
    }
...
fn cache_scan_free(
    ...
    if flags & (1) << 3 != 0 {
        ...
        crecp = *up;
        while !crecp.is_null() {
            if crecp.flags & (1) << 3 != 0 && hostname_isequal(cache_get_name(crecp), name) != 0 {
                /* Don't delete DNSSEC in favour of a CNAME, they can co-exist */
                if flags & crecp.flags & ((1) << 7 | (1) << 8 | (1) << 30) != 0
                    || (crecp.flags | flags) & (1) << 11 != 0
                        && crecp.flags & ((1) << 12 | (1) << 14) == 0
                {
                    if crecp.flags & ((1) << 6 | (1) << 4 | (1) << 13) != 0 {
                        return crecp;
                    }
                    *up = crecp.hash_next;
                    ...
                    current_block_18 = 4956146061682418353;
                }
            } else {
                current_block_18 = 4956146061682418353;
            }
            match current_block_18 {
                4956146061682418353 => {
                    if is_expired(now, crecp) != 0 || is_outdated_cname_pointer(crecp) != 0 {
                        *up = crecp.hash_next;
                        if crecp.flags & ((1) << 6 | (1) << 4 | (1) << 13) == 0 {
                            cache_unlink(crecp);
                            cache_free(crecp);
        ...
        let mut i: i32 = 0;
        let mut addrlen: i32 = if flags & (1) << 8 != 0 { 16 } else { 4 };
        i = 0;
        while i < hash_size {
            crecp = hash_table.offset(i);
            up = &mut hash_table.offset(i);
            while !crecp.is_null() && (crecp.flags & (1) << 2 != 0 || crecp.flags & (1) << 0 == 0) {
...
fn really_insert(
    ...
    let mut freed_all: i32 = (flags & (1) << 2);
    ...
    if !new.is_null() {
        ...
        if flags & ((1) << 7 | (1) << 8) != 0 && flags & (1) << 3 != 0 && !addr.is_null() {
            if flags & (1) << 7 != 0
                && new.flags & (1) << 7 != 0
                && new.addr.addr4.s_addr == addr.addr4.s_addr
            {
                ...
                if flags & (1) << 8 != 0
                    && new.flags & (1) << 8 != 0
                    && ({
                        let mut __a: *const In6Addr = &mut new.addr.addr6;
                        let mut __b: *const In6Addr = &mut addr.addr6;
    ...
    if target_crec.is_null() {
        loop {
            ...
            if freed_all != 0 {
                ...
                cache_scan_free(0, 0, class, now, 0, 0, 0);
                freed_all = 1
            }
        }
    }
    ...
    if !name.is_null() && strlen(name) > (50 - 1) {
        if !big_free.is_null() {
            big_name = big_free;
            big_free = big_free.next
        } else if bignames_left == 0 && flags & ((1) << 14 | (1) << 12) == 0 || {
            big_name = whine_malloc(::std::mem::size_of::<BigName>());
            big_name.is_null()
        } {
            insert_error = 1;
            return 0;
    ...
    while !new_chain.is_null() {
        ...
        if is_outdated_cname_pointer(new_chain) != 0 {
            ...
            daemon.metrics[METRIC_DNS_CACHE_INSERTED] =
                daemon.metrics[METRIC_DNS_CACHE_INSERTED].wrapping_add(1);
            ...
            if daemon.pipe_to_parent != -(1) {
                ...
                read_write(daemon.pipe_to_parent, name, m, 0);
                read_write(
                    daemon.pipe_to_parent,
                    &mut new_chain.ttd,
                ...
                if flags & ((1) << 7 | (1) << 8 | (1) << 12 | (1) << 14 | (1) << 30) != 0 {
                    read_write(
                        daemon.pipe_to_parent,
                        &mut new_chain.addr,
...
pub fn cache_recv_insert(mut now: time::Instant, mut fd: i32) -> i32 {
    let mut m: isize = 0;
    ...
    cache_start_insert();
    loop {
        if read_write(fd, &mut m, ::std::mem::size_of::<isize>(), 1) == 0 {
            return 0;
        }
        if m == -(1) {
            cache_end_insert();
        ...
        if read_write(fd, daemon.namebuff, m, 1) == 0
            || read_write(fd, &mut ttd, ::std::mem::size_of::<time::Instant>(), 1) == 0
            || read_write(fd, &mut flags, ::std::mem::size_of::<libc::c_uint>(), 1) == 0
        {
            return 0;
        }
        *daemon.namebuff.offset(m) = 0;
        ttl = difftime(ttd, now);
        if flags & ((1) << 7 | (1) << 8 | (1) << 12 | (1) << 14 | (1) << 30) != 0 {
            let mut class: u16 = 1;
            if read_write(fd, &mut addr, ::std::mem::size_of::<NetAddress>(), 1) == 0 {
                return 0;
            }
            if flags & (1) << 30 != 0 && flags & (1) << 5 == 0 && {
                addr.srv.target = blockdata_read(fd, addr.srv.targetlen);
                addr.srv.target.is_null()
            } {
                return 0;
            }
            crecp = really_insert(daemon.namebuff, &mut addr, class, now, ttl, flags)
        } else if flags & (1) << 11 != 0 {
            let mut newc: Crec = really_insert(daemon.namebuff, 0, 1, now, ttl, flags);
...
pub fn cache_find_non_terminal(mut name: &String, mut now: time::Instant) -> i32 {
    let mut crecp: Crec;
...
pub fn cache_find_by_name(
    ...
    if !crecp.is_null() {
        ...
        while !crecp.is_null() {
            ...
            if is_expired(now, crecp.unwrap()) == 0 && is_outdated_cname_pointer(crecp) == 0 {
                    ...
                    && hostname_isequal(cache_get_name(crecp), name) != 0
                {
                    ...
                    /* Move all but the first entry up the hash chain
                       ...
                       order invariants.
                    */
                    if !insert.is_null() && crecp.flags & ((1) << 2 | (1) << 0) == ins_flags {
                        *up = crecp.hash_next;
...
pub fn cache_find_by_addr(
    mut crecp: Option<&mut Vec<Crec>>,
    mut addr: &mut NetAddress,
    mut now: time::Instant,
    mut prot: u32,
) -> Crec {
    let mut ans: Crec = Default::default();
    let mut addrlen: i32 = if prot == (1 << 8) { 16 } else { 4 };
    if crecp.is_some() {
        ...
        while i < hash_size {
            ...
            while !crecp.is_null() && crecp.flags & (1) << 2 != 0 {
                if is_expired(now, crecp) == 0 {
    ...
    if !ans.is_null()
        && ans.flags & (1) << 2 != 0
        && ans.flags & prot != 0
        && memcmp(&mut ans.addr, addr, addrlen) == 0
    {
        return ans;
    }
    return 0;
...
fn add_hosts_entry(
    ...
    let mut lookup: Crec = cache_find_by_name(
        0,
    ...
    if !lookup.is_null()
        && lookup.flags & (1) << 6 != 0
        && memcmp(&mut lookup.addr, addr, addrlen) == 0
    {
        // free(cache);
        return;
    }
    ...
    */
    if !rhash.is_null() {
        ...
        while i < addrlen {
            j = j
                .wrapping_mul(2)
                .wrapping_add((addr).offset(i))
                .wrapping_rem(hashsz);
            i += 1
        }
        lookup = *rhash.offset(j);
        while !lookup.is_null() {
            if lookup.flags & cache.flags & ((1) << 7 | (1) << 8) != 0 &&
        ...
        lookup = cache_find_by_addr(0, addr, 0, cache.flags & ((1) << 7 | (1) << 8));
        if !lookup.is_null() && lookup.flags & (1) << 6 != 0 {
            cache.flags &= !((1) << 2)
        }
    }
    cache.uid = index;
    memcpy(&mut cache.addr, addr, addrlen);
    cache_hash(cache);
...
fn eatspace(mut f: &mut FILE) -> i32 {
    ...
    loop {
        ...
        if *(*__ctype_b_loc()).offset(c) & _ISSPACE == 0 {
            ungetc(c, f);
            return nl;
        }
        if c == '\n' as i32 {
...
fn gettok(mut f: &mut FILE, mut token: &mut String) -> i32 {
    let mut c: i32 = 0;
    let mut count: i32 = 0;
    loop {
        c = getc(f);
        if c == -(1) {
            return if count == 0 { -(1) } else { 1 };
        }
        if *(*__ctype_b_loc()).offset(c) & _ISSPACE != 0 || c == '#' as i32 {
            ungetc(c, f);
            return eatspace(f);
        }
        if count < 1025 - 1 {
...
pub fn read_hostsfile(
    ...
    loop {
        ...
        if inet_pton(2, token, &mut addr) > 0 {
            flags = (1) << 6 | (1) << 0 | (1) << 3 | (1) << 2 | (1) << 7;
            addrlen = 4;
            domain_suffix = get_domain(addr.addr4)
        } else if inet_pton(10, token, &mut addr) > 0 {
            flags = (1) << 6 | (1) << 0 | (1) << 3 | (1) << 2 | (1) << 8;
            addrlen = 16;
            domain_suffix = get_domain6(&mut addr.addr6)
        } else {
            my_syslog(3, "bad address at %s line %d", filename, lineno);
            while atnl == 0 {
                atnl = gettok(f, token)
            }
            lineno += atnl;
...
pub fn cache_reload(daemon: &mut DnsmasqDaemon) {
    ...
    for hr in daemon.host_records {
        for nl in hr.names {
            ...
            if hr.flags & 1 != 0 && {
                ...
                cache.ttd = hr.ttl;
                cache.flags =
                    (1) << 6 | (1) << 0 | (1) << 3 | (1) << 2 | (1) << 8 | (1) << 1 | (1) << 13;
                add_hosts_entry(cache, &mut hr.addr6, 16, 1, daemon.packet, revhashsz);
            }
            nl = nl.next
        }
        hr = hr.next
    ...
    if daemon.options[(4).wrapping_div((::std::mem::size_of::<libc::c_uint>()).wrapping_mul(8))]
        & (1) << (4).wrapping_rem((::std::mem::size_of::<libc::c_uint>()).wrapping_mul(8))
        != 0
        && daemon.addn_hosts.is_null()
    {
        if daemon.cachesize > 0 {
            my_syslog(6, "cleared cache");
        }
    } else {
        if daemon.options[4] == 0 {
            total_size = read_hostsfile("/etc/hosts", 2, total_size, daemon.packet, revhashsz)
        }
        ...
        while !ah.is_null() {
            if ah.flags & 2 == 0 {
                total_size =
                    read_hostsfile(ah.fname, ah.index, total_size, daemon.packet, revhashsz)
            }
            ah = ah.next
        }
    }
    /* Make non-terminal records for all locally-define RRs */
    lrec.flags = (1) << 3 | (1) << 13 | (1) << 1 | (1) << 0;
    txt = daemon.txt;
    while !txt.is_null() {
    ...
    set_dynamic_inotify(8, total_size, daemon.packet, revhashsz);
}

pub fn a_record_from_hosts(mut name: &mut String, mut now: time::Instant) -> NetAddress {
    let mut crecp: Crec = 0;
    let mut ret: NetAddress = NetAddress::new();
    loop {
        crecp = cache_find_by_name(crecp, name, now, (1) << 7);
        if crecp.is_null() {
            break;
        }
...
pub fn cache_add_dhcp_entry(
    ...
    let mut addrlen: usize = ::std::mem::size_of::<NetAddress>();
    if prot == 10 {
        flags = (1) << 8;
        addrlen = ::std::mem::size_of::<In6Addr>()
    }
    inet_ntop(prot, host_address, daemon.addrbuff, 46);
    loop {
        crec = cache_find_by_name(crec, host_name, 0, flags | (1) << 11);
        if crec.is_null() {
            break;
        }
        /* check all addresses associated with name */
        if crec.flags & ((1) << 6 | (1) << 13) != 0 {
            ...
            // else if memcmp(&mut crec.addr, host_address, addrlen) == 0 {
            else if crec.addr == 0 && host_addr == 0 {
                in_hosts = 1
            } else {
                fail_crec = crec
            }
    ...
    if !fail_crec.is_null() {
        inet_ntop(prot, &mut fail_crec.addr, daemon.namebuff, 1025);
        // my_syslog((3) << 3 | 4,
    ...
    crec = cache_find_by_addr(0, host_address, 0, flags);
    if !crec.is_null() {
        if crec.flags & (1) << 5 != 0 {
            flags |= (1) << 2;
            cache_scan_free(0, host_address, 1, 0, flags, 0, 0);
        }
    } else {
        flags |= (1) << 2
    }
...
fn make_non_terminals(mut source: Crec) {
    ...
    while !crecp.is_null() {
        ...
        if is_outdated_cname_pointer(crecp) == 0
            ...
            && crecp.flags & ((1) << 7 | (1) << 8 | (1) << 11 | (1) << 30 | (1) << 12 | (1) << 14)
                == 0
            && hostname_isequal(name, cache_get_name(crecp)) != 0
        {
            *up = crecp.hash_next;
    ...
    loop {
        ...
        if !crecp.is_null() {
            ...
            if source.flags & (1) << 4 != 0 && !dhcp_spare.is_null() {
                crecp = dhcp_spare;
                dhcp_spare = dhcp_spare.next
            } else {
                // crecp =
...
pub fn cache_make_stat(mut t: &mut TxtRecord) -> i32 {
    ...
    match t.stat {
        ...
        7 => {
            ...
            serv = daemon.servers;
            while !serv.is_null() {
                if serv.flags & (2 | 4 | 512 | 1024 | 2048) == 0 {
                    ...
                    serv1 = serv;
                    while !serv1.is_null() {
                        if serv1.flags & (2 | 4 | 512 | 1024 | 2048) == 0
                            && NetAddress_isequal(&mut serv.addr, &mut serv1.addr) != 0
                        {
                            serv1.flags |= 512;
                            queries = queries.wrapping_add(serv1.queries);
                            failed_queries = failed_queries.wrapping_add(serv1.failed_queries)
                        }
                        serv1 = serv1.next
                    }
                    port = prettyprint_addr(&mut serv.addr, daemon.addrbuff);
                    let fresh11 = p;
                    p = p.offset(1);
                    lenp = fresh11;
                    bytes_avail = (bufflen - p.wrapping_offset_from(buff));
                    // bytes_needed =
                    ...
                    //              failed_queries);
                    // bytes_needed = format!()
                    p = format!("{}{} {} {}", daemon.addrbuff, port, queries, failed_queries);
            ...
            t.txt = buff;
            t.len = p.wrapping_offset_from(buff);
            return 1;
        }
        _ => {}
    }
...
fn sanitise(mut name: &mut String) -> &mut String {
    let mut r: Vec<u8> = Vec::new();
    if !name.is_null() {
        r = name;
        while *r != 0 {
            if *(*__ctype_b_loc()).offsetr & _ISPRINT == 0 {
...
pub fn dump_cache(mut now: time::Instant) {
    ...
    if daemon.options[6] != 0 || daemon.options[2] != 0 {
        let mut cache: Crec = 0;
        let mut i: i32 = 0;
        // my_syslog(6,
        ...
        while i < hash_size {
            cache = hash_table.offset(i);
            while !cache.is_null() {
                let mut t: &mut String = &mut String;
                let mut a: &mut String = daemon.addrbuff;
                let mut p: &mut String = daemon.namebuff;
                let mut n: &mut String = cache_get_name(cache);
                *a = 0;
                if strlen(n) == 0 && cache.flags & (1) << 2 == 0 {
                    n = "<Root>"
                }
                p = p.offset(sprintf(p, "%-30.30s ", sanitise(n)));
                if cache.flags & (1) << 11 != 0 && is_outdated_cname_pointer(cache) == 0 {
                    a = sanitise(cache_get_cname_target(cache))
                } else if cache.flags & (1) << 30 != 0 && cache.flags & (1) << 5 == 0 {
                    let mut targetlen: i32 = cache.addr.srv.targetlen;
                    // let mut len: isize =
                    //     sprintf(a,
                    ...
                    a = format!(
                        "{} {} {}",
                        cache.addr.srv.priority, cache.addr.srv.weight, cache.addr.srv.srvport
                    );
                    let mut len = a.len();
                    if targetlen > 40 - len {
                        targetlen = (40 - len)
                    }
                    blockdata_retrieve(cache.addr.srv.target, targetlen, a.offset(len));
                    *a.offset((len + targetlen)) = 0
                } else if cache.flags & (1) << 5 == 0 || cache.flags & (1) << 3 == 0 {
                    a = daemon.addrbuff;
                    if cache.flags & (1) << 7 != 0 {
                        a = inet_ntop(2, &mut cache.addr).unwrap();
                    } else if cache.flags & (1) << 8 != 0 {
                        a = inet_ntop(10, &mut cache.addr).unwrap();
                    }
                }
                if cache.flags & (1) << 7 != 0 {
                    t = "4"
                    ...
                    t = "6"
                } else if cache.flags & (1) << 11 != 0 {
                    t = "C"
                } else if cache.flags & (1) << 30 != 0 {
                    t = "V"
                }
                p = p.offset(sprintf(
                    p,
                    "%-40.40s %s%s%s%s%s%s%s%s%s  ",
                    a,
                    t,
                    if cache.flags & (1) << 3 != 0 {
                        "F"
                    } else {
                        " "
                    },
                    if cache.flags & (1) << 2 != 0 {
                        "R"
                    } else {
                        " "
                    },
                    if cache.flags & (1) << 0 != 0 {
                        "I"
                    } else {
                        " "
                    },
                    ...
                    if cache.flags & (1) << 5 != 0 {
                        "N"
                    } else {
                        " "
                    },
                    if cache.flags & (1) << 10 != 0 {
                        "X"
                    } else {
                        " "
                    },
                    if cache.flags & (1) << 6 != 0 {
                        "H"
                    } else {
                        " "
                    },
                    if cache.flags & (1) << 15 != 0 {
                        "V"
                    } else {
                        " "
                    },
                ...
                *p.offset(-(1)) = 0; /* strlen("type=xxxxx") */
                // my_syslog(6,
                //           "%s" ,
...
pub fn record_source(mut index: u32) -> String {
...
pub fn querystr(mut desc: &mut String, mut type_0: u16) -> &mut String {
    let mut i: u32 = 0;
    ...
    i = 0;
    while (i)
        < (::std::mem::size_of::<[C2rustUnnamed10; 40]>())
            .wrapping_div(::std::mem::size_of::<C2rustUnnamed10>())
    {
    ...
    if !desc.is_null() {
        len += 2;
        len = (len).wrapping_add(strlen(desc))
    }
    len += 1;
    ...
    if !buff.is_null() {
        if !desc.is_null() {
            if !types.is_null() {
                ...
                buff = format!("{}[type={}]", desc, type_0);
            }
        } else if !types.is_null() {
            buff = format!("<{}>", types);
        } else {
            buff = format!("type={}", type_0);
        }
    }
    return if !buff.is_null() { buff } else { "" };
}
