...
*/
use crate::defines::{CondDomain, DnsmasqDaemon, NetAddress};
use crate::util::{addr6part, is_same_net6};

fn search_domain(mut addr: NetAddress, mut c: &mut CondDomain) -> CondDomain {
    while !c.is_null() {
        if c.is6 == 0
...
pub fn get_domain(mut addr: NetAddress) -> &mut String {
...
fn search_domain6(mut addr: &mut In6Addr, mut c: &mut CondDomain) -> Option<CondDomain> {
    let mut addrpart: u64 = addr6part(addr);
    while !c.is_null() {
        if c.is6 != 0
            && s_same_net6(addr, &mut c.start6, 64) != 0
...
pub fn get_domain6(mut addr: &mut In6Addr) -> &mut String {
    let mut c: CondDomain = 0;
    if !addr.is_null() && {
