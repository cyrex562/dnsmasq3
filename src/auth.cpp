        ...
        int local_query, int do_bit, int have_pseudoheader)
{
    ...
    for (q = ntohs(header->qdcount); q!=0; q--) {
        ...
        if (qtype==T_PTR && flag) {
            ...
            if (intr) {
                if (local_query || in_zone(zone, intr->name, nullptr)) {
                    found = 1;
                    log_query(flag | F_REVERSE | F_CONFIG, intr->name, &addr, nullptr);
                    if (add_resource_record(header, limit, &trunc, nameoffset, &ansp,
                            daemon->auth_ttl, nullptr,
                            T_PTR, C_IN, "d", intr->name))
                        anscount++;
                }
            }

            if ((crecp = cache_find_by_addr(nullptr, &addr, now, flag)))
                do {
                    ...
                    else if (crecp->flags & (F_DHCP | F_HOSTS) && (local_query || in_zone(zone, name, nullptr))) {
            ...
            if (found)
                ...
                log_query(flag | F_NEG | F_NXDOMAIN | F_REVERSE | (auth ? F_AUTH : 0), nullptr, &addr, nullptr);
            ...
            if (!rec->issrv && (rc = hostname_issubdomain(name, rec->name))) {
                ...
                if (rc==2 && qtype==T_MX) {
                    ...
                    if (add_resource_record(header, limit, &trunc, nameoffset, &ansp, daemon->auth_ttl,
                            nullptr, T_MX, C_IN, "sd", rec->weight, rec->target))
                        anscount++;
                }
            }

        for (move = nullptr, up = &daemon->mxnames, rec = daemon->mxnames; rec; rec = rec->next)
            if (rec->issrv && (rc = hostname_issubdomain(name, rec->name))) {
                nxdomain = 0;
        ...
        for (txt = daemon->rr; txt; txt = txt->next)
            if ((rc = hostname_issubdomain(name, txt->name))) {
                nxdomain = 0;
                if (rc==2 && txt->_class == qtype)
                {
                    found = 1;
                    log_query(F_CONFIG | F_RRNAME, name, nullptr, querystr(nullptr, txt->
                    _class));
                    if (add_resource_record(header, limit, &trunc, nameoffset, &ansp, daemon->auth_ttl,
                            nullptr, txt->
                        _class, C_IN, "t", txt->len, txt->txt))
                    anscount++;
                }
            }

        for (txt = daemon->txt; txt; txt = txt->next)
            if (txt->_class == C_IN && (rc = hostname_issubdomain(name, txt->name)))
        {
            nxdomain = 0;
            if (rc==2 && qtype==T_TXT) {
                found = 1;
                ...
                if (add_resource_record(header, limit, &trunc, nameoffset, &ansp, daemon->auth_ttl,
                        nullptr, T_TXT, C_IN, "t", txt->len, txt->txt))
                    anscount++;
            }
        }
      ...
      flag = F_IPV6;
        ...
        if (!cut) {
            ...
            if (qtype==T_SOA) {
                ...
                else
              {
                ...
                log_query(F_RRNAME | F_AUTH, zone->domain, nullptr, "<AXFR>");
            }
            else if (qtype==T_NS) {
                auth = 1;
                ns = 1; /* inhibits auth section */
                found = 1;
                log_query(F_RRNAME | F_AUTH, zone->domain, nullptr, "<NS>");
            }
        }
        ...
        if (!option_bool(OPT_DHCP_FQDN) && cut) {
            *cut = 0; /* remove domain part */
            ...
            if (!strchr(name, '.') && (crecp = cache_find_by_name(nullptr, name, now, F_IPV4 | F_IPV6))) {
        ...
        if ((crecp = cache_find_by_name(nullptr, name, now, F_IPV4 | F_IPV6))) {
        ...
        if (nxdomain) {
            ...
            */
            for (wclen = 0, candidate = nullptr, a = daemon->cnames; a; a = a->next)
                if (a->alias[0]=='*') {
                    char* test = name;
            ...
            if (candidate) {
                log_query(F_CONFIG | F_CNAME, name, nullptr, nullptr);
            ...
            log_query(flag | F_NEG | (nxdomain ? F_NXDOMAIN : 0) | F_FORWARD | F_AUTH, name, nullptr, nullptr);
        }
    ...
    if (auth && zone) {
        ...
        if (((anscount==0 && !ns) || soa) &&
                add_resource_record(header, limit, &trunc, 0, &ansp,
                        daemon->auth_ttl, nullptr, T_SOA, C_IN, "ddlllll",
        ...
        if (anscount!=0 || ns) {
            ...
            if (add_resource_record(header, limit, &trunc, -offset, &ansp,
                    daemon->auth_ttl, nullptr, T_NS, C_IN, "d", offset==0 ? authname : nullptr, daemon->authserver)) {
                if (offset==0)
                    offset = newoffset;
                if (ns)
                    anscount++;
            ...
            if (!subnet)
                for (secondary = daemon->secondary_forward_server; secondary; secondary = secondary->next)
                    if (add_resource_record(header, limit, &trunc, offset, &ansp,
                            daemon->auth_ttl, nullptr, T_NS, C_IN, "d", secondary->name)) {
                        if (ns)
                            anscount++;
                        else
                            authcount++;
        ...
        if (axfr) {
                ...
                if (in_zone(zone, rec->name, &cut)) {
                    ...
                    if (rec->issrv) {
                        if (add_resource_record(header, limit, &trunc, -axfroffset, &ansp, daemon->auth_ttl,
                                nullptr, T_SRV, C_IN, "sssd", cut ? rec->name : nullptr,
                                rec->priority, rec->weight, rec->srvport, rec->target))
                        ...
                        if (add_resource_record(header, limit, &trunc, -axfroffset, &ansp, daemon->auth_ttl,
                                nullptr, T_MX, C_IN, "sd", cut ? rec->name : nullptr, rec->weight, rec->target))
                            anscount++;
                    }
            ...
            for (txt = daemon->rr; txt; txt = txt->next)
                if (in_zone(zone, txt->name, &cut)) {
                    ...
                    if (add_resource_record(header, limit, &trunc, -axfroffset, &ansp, daemon->auth_ttl,
                            nullptr, txt->
                        _class, C_IN, "t", cut ? txt->name : nullptr, txt->len, txt->txt))
                    anscount++;
            ...
            for (txt = daemon->txt; txt; txt = txt->next)
                if (txt->_class == C_IN && in_zone(zone, txt->name, &cut))
            {
                if (cut)
                    *cut = 0;

                if (add_resource_record(header, limit, &trunc, -axfroffset, &ansp, daemon->auth_ttl,
                        nullptr, T_TXT, C_IN, "t", cut ? txt->name : nullptr, txt->len, txt->txt))
                    anscount++;
            ...
            for (na = daemon->naptr; na; na = na->next)
                if (in_zone(zone, na->name, &cut)) {
                    ...
                    if (add_resource_record(header, limit, &trunc, -axfroffset, &ansp, daemon->auth_ttl,
                            nullptr, T_NAPTR, C_IN, "sszzzd", cut ? na->name : nullptr,
                            na->order, na->pref, na->flags, na->services, na->regexp, na->replace))
                        anscount++;
                    ...
                    for (addrlist = intr->addr; addrlist; addrlist = addrlist->next)
                        if (!(addrlist->flags & ADDRLIST_IPV6) &&
                                ...
                                add_resource_record(header, limit, &trunc, -axfroffset, &ansp,
                                        daemon->auth_ttl, nullptr, T_A, C_IN, "4", cut ? intr->name : nullptr,
                                        &addrlist->addr))
                            anscount++;
            ...
            for (a = daemon->cnames; a; a = a->next)
                if (in_zone(zone, a->alias, &cut)) {
                    ...
                    if (add_resource_record(header, limit, &trunc, -axfroffset, &ansp,
                            daemon->auth_ttl, nullptr,
                            T_CNAME, C_IN, "d", cut ? a->alias : nullptr, name))
                        anscount++;
                }
                ...
                if ((crecp->flags & (F_IPV4 | F_IPV6)) &&
                    ...
                    if ((crecp->flags & F_HOSTS) || (((crecp->flags & F_DHCP) && option_bool(OPT_DHCP_FQDN)))) {
                        ...
                        if (in_zone(zone, name, &cut) &&
                            ...
                            if (add_resource_record(header, limit, &trunc, -axfroffset, &ansp,
                                    daemon->auth_ttl, nullptr, qtype, C_IN,
                                    (crecp->flags & F_IPV4) ? "4" : "6", cut ? name : nullptr, &crecp->addr))
                                anscount++;
                        }
                    }
                }
    ...
    if (have_pseudoheader)
        return add_pseudoheader(header, ansp-(unsigned char*) header, (unsigned char*) limit, daemon->edns_pktsz, 0,
                nullptr, 0, do_bit, 0);
