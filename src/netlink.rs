...
//#endif
    ...
    if ((daemon->netlinkfd = socket(AF_NETLINK, SOCK_RAW, NETLINK_ROUTE))!=-1) {
        if (bind(daemon->netlinkfd, (struct sockaddr*) &addr, sizeof(addr))==-1) {
            addr.nl_groups = 0;
            if (errno!=EPERM
                    || bind(daemon->netlinkfd, (struct sockaddr*) &addr, sizeof(addr))
                            ==-1)
                daemon->netlinkfd = -1;
        }
    }
    ...
    if (daemon->netlinkfd==-1
        ...
        die(_("cannot create netlink socket: %s"), nullptr, EC_MISC);
    }
...
using callback_t = int* (int, char*, char*, size_t, void*);
                        ...
                        while (RTA_OK(rta, len1)) {
                            if (rta->rta_type==IFA_ADDRESS) {
                                ...
                                struct ifa_cacheinfo
                                        * ifc = (struct ifa_cacheinfo*) (rta+1);
...
//#endif
                ...
                size_t maclen = 0;
                char* inaddr = nullptr, * mac = nullptr;
...
//#ifdef HAVE_DHCP6
                ...
                if (mac && callback_ok
                        ...
                        && !((*callback)((int) link->ifi_index,
                                (unsigned int) link->ifi_type,
                                mac,
                                maclen,
                                parm))) {
                    callback_ok = 0;
