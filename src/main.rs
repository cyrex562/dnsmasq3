...
use libc::{_SC_OPEN_MAX, c_char, c_long, free, gid_t, group, malloc, O_RDWR, open, passwd, SIG_IGN, sigaction, SIGALRM, SIGCHLD, sigemptyset, SIGHUP, SIGINT, SIGPIPE, sigset_t, SIGTERM, SIGUSR1, SIGUSR2, sysconf, time_t, uid_t, umask};
use crate::app_context::AppContext;
use crate::dhcp_context::DhcpContext;
use crate::dhcp_relay::DhcpRelay;
use crate::dns_protocol::{MAXDNAME, PACKETSZ, RRFIXEDSZ};
use crate::dnsmasq_defines::{OPT_CLEVERBIND, OPT_CMARK_ALST_EN, OPT_DNSSEC_VALID, OPT_EXTRALOG, OPT_NOWILD};
...
fn main() {
    ...
    let mut need_cap_net_admin = false;
    let mut need_cap_net_raw = false;
    let mut need_cap_net_bind_service = false;
    let mut bound_device = String::new();
    let mut did_bind = false;
    let mut serv: *mut Server = null_mut();
    let mut netlink_warn = String::new();
    // end cap net raw only
    let mut bind_fallback: i32 = 0;
    let mut context: *mut DhcpContext = null_mut();
    let mut relay: *mut DhcpRelay = null_mut();
    let mut tftp_prefix_missing = false;
...
// // #endif
  ...
  rand_init(&mut app_ctx.rand_ctx); /* Must precede read_opts() */
...
// #endif
  ...
  app_ctx.daemon.packet_buff_sz = app_ctx.daemon.edns_pktsz + MAXDNAME + RRFIXEDSZ;
    unsafe { app_ctx.daemon.packet = malloc(app_ctx.daemon.packet_buff_sz); }

  if option_bool(OPT_EXTRALOG) {
      unsafe { app_ctx.daemon.addrbuff2 = malloc(ADDRSTRLEN); }
  }
  ...
  if option_bool(OPT_DNSSEC_VALID)
    {
	 ...
	 code before we knew if we're in DNSSEC mode, so reallocate here. */
        unsafe { free(app_ctx.daemon.namebuff); }
        unsafe { daemon.namebuff = malloc(MAXDNAME * 2); }
        unsafe { daemon.keyname = malloc(MAXDNAME * 2); }
        unsafe { daemon.workspacename = malloc(MAXDNAME * 2); }
      /* one char flag per possible RR in answer section (may get extended). */
      daemon.rr_status_sz = 64;
        unsafe { daemon.rr_status = malloc(sizeof(*daemon.rr_status) * daemon.rr_status_sz); }
    }
// #endif
...
// #if defined(HAVE_CONNTRACK) && defined(HAVE_UBUS)
  ...
  if option_bool(OPT_CMARK_ALST_EN) && !daemon.workspacename {
      unsafe { daemon.workspacename = malloc(MAXDNAME); }
  }
// #endif
...
// #endif
  ...
  /* Ensure that at least stdin, stdout and stderr (fd 0, 1, 2) exist,
    ...
    let dev_null = CString::from("/dev/null");
  for i in 0 .. 3 {
      unsafe {
          open(dev_null.as_ptr(), O_RDWR);
      }
  }
...
// #endif HAVE_INOTIFY
  if daemon.dynamic_dirs {
      // panic!(_("dhcp-hostsdir, dhcp-optsdir and hostsdir are not supported on this platform"), NULL, EC_BADCONF);
...
// #ifdef HAVE_DNSSEC
//       struct ds_config *ds;
        let mut ds: *mut ds_config = null_mut();

      /* Must have at least a root trust anchor, or the DNSSEC code
	 can loop forever. */
      for (ds = app_ctx.daemon.ds; ds; ds = ds.next)
	if ds.name[0] == 0
	  break;
...
// #endif
  ...
  if (daemon.port != 0)
	...
	daemon.numrrand = max_fd/3;
      ...
      daemon.randomsocks = malloc(daemon.numrrand * sizeof(struct randfd));
    }
