...
pub use self::dnsmasq_h::{u32_0, all_addr, C2RustUnnamed_0, C2RustUnnamed_1,
                          ...
                          dhcp_relay, dnsmasq_daemon, whine_malloc, my_syslog,
                          read_write};
...
pub use self::config_h::KEYBLOCK_LEN;
...
pub unsafe extern "C" fn blockdata_init() {
    keyblock_free = NULL_0 as *mut blockdata;
...
unsafe extern "C" fn blockdata_alloc_real(mut fd: libc::c_int,
    ...
    let mut block = 0 as *mut blockdata;
    let mut ret = NULL_0 as *mut blockdata;
    ...
    while len > 0 as libc::c_int as libc::c_ulong {
        ...
        if !keyblock_free.is_null() {
            ...
            blockdata_free(ret);
            return NULL_0 as *mut blockdata
        }
        if blockdata_hwm < blockdata_count { blockdata_hwm = blockdata_count }
        blen =
            if len > KEYBLOCK_LEN as libc::c_ulong {
                KEYBLOCK_LEN as libc::c_ulong
            } else { len };
        if !data.is_null() {
        ...
        *prev = block;
        prev = &mut (*block).next;
        (*block).next = NULL_0 as *mut blockdata
    }
    return ret;
}
#[no_mangle]
...
pub unsafe extern "C" fn blockdata_free(mut blocks: *mut blockdata) {
    let mut tmp = 0 as *mut blockdata;
    if !blocks.is_null() {
        tmp = blocks;
...
pub unsafe extern "C" fn blockdata_retrieve(mut block: *mut blockdata,
 ...
 -> *mut libc::c_void {
    let mut blen: size_t = 0;
    let mut b = 0 as *mut blockdata;
    let mut new = 0 as *mut libc::c_void;
    let mut d = 0 as *mut libc::c_void;
    static mut buff_len: libc::c_uint = 0 as libc::c_int as libc::c_uint;
    static mut buff: *mut libc::c_uchar = NULL_0 as *mut libc::c_uchar;
    if data.is_null() {
        if len > buff_len as libc::c_ulong {
            new = whine_malloc(len);
            if new.is_null() { return NULL_0 as *mut libc::c_void }
...
pub unsafe extern "C" fn blockdata_read(mut fd: libc::c_int, mut len: size_t)
 -> *mut blockdata {
    return blockdata_alloc_real(fd, NULL_0 as *mut libc::c_char, len);
}
