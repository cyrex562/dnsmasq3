...
pub mod sys_types_h {
    ...
    use super::types_h::{__ino64_t, __dev_t, __mode_t, __off64_t, __pid_t};
}
...
pub mod ip_h {
    ...
    pub const IPVERSION: libc::c_int = 4 as libc::c_int;
    #[c2rust::src_loc = "294:9"]
    pub const IPDEFTTL: libc::c_int = 64 as libc::c_int;
...
#[c2rust::header_src = "/usr/include/netinet/ip6.h:17"]
pub mod ip6_h {
    #[derive(Copy, Clone)]
    #[repr(C)]
    #[c2rust::src_loc = "24:8"]
...
#[c2rust::header_src = "/usr/include/x86_64-linux-gnu/sys/time.h:17"]
pub mod time_h {
    use super::struct_timeval_h::timeval;
    extern "C" {
        #[no_mangle]
        #[c2rust::src_loc = "66:1"]
        pub fn gettimeofday(__tv: *mut timeval, __tz: *mut libc::c_void)
         -> libc::c_int;
    }
}
...
pub use self::sys_types_h::{ino_t, dev_t, mode_t, off_t, pid_t};
...
pub use self::in_h::{sockaddr_in6, in6_addr, C2RustUnnamed, in_port_t,
                     ...
                     IPPROTO_ICMP, IPPROTO_IP, IPPROTO_UDP_0};
pub use self::stdint_uintn_h::{uint32_t, uint16_t, uint8_t};
pub use self::dnsmasq_h::{u16_0, u32_0, all_addr, C2RustUnnamed_2,
                          ...
                          dhcp_relay, dnsmasq_daemon, EC_FILE, read_write,
                          die, my_syslog};
...
pub use self::ip_h::{ip, IPVERSION, IPDEFTTL};
...
pub use self::ip6_h::{ip6_hdr, C2RustUnnamed_1, ip6_hdrctl};
...
pub use self::fcntl_h::{S_IRUSR, S_IWUSR, SEEK_CUR, creat, open};
...
pub use self::fcntl_linux_h::{O_APPEND, O_RDWR};
    ...
    if stat((*dnsmasq_daemon).dump_file, &mut buf) == -(1 as libc::c_int) {
        ...
        if errno != ENOENT ||
            ...
            die(b"cannot create %s: %s\x00" as *const u8 as
                ...
                (*dnsmasq_daemon).dump_file, EC_FILE);
        }
    } else {
        (*dnsmasq_daemon).dumpfd =
            open((*dnsmasq_daemon).dump_file, O_APPEND | O_RDWR);
            ...
            if header.magic_number != 0xa1b2c3d4 as libc::c_uint {
                ...
                while read_write((*dnsmasq_daemon).dumpfd,
                    ...
                    lseek((*dnsmasq_daemon).dumpfd,
                          pcap_header.incl_len as __off64_t, SEEK_CUR);
...
pub unsafe extern "C" fn dump_packet(mut mask: libc::c_int,
    ...
    let mut udp = udphdr{uh_sport: 0, uh_dport: 0, uh_ulen: 0, uh_sum: 0,};
    let mut pcap_header =
        pcaprec_hdr_s{ts_sec: 0, ts_usec: 0, incl_len: 0, orig_len: 0,};
    let mut time = timeval{tv_sec: 0, tv_usec: 0,};
    let mut i: u32_0 = 0;
    let mut sum: u32_0 = 0;
    let mut iphdr = 0 as *mut libc::c_void;
    ...
    if family == AF_INET6 {
        ...
        ip6.ip6_ctlun.ip6_un1.ip6_un1_nxt = IPPROTO_UDP_0 as uint8_t;
        ...
        while i < IN6ADDRSZ as libc::c_uint {
            sum =
                (sum as
        ...
        ip.set_ip_v(IPVERSION as libc::c_uint);
        ...
        ip.ip_len =
            __bswap_16((::std::mem::size_of::<ip>() as
                           ...
                           as __uint16_t);
        ip.ip_ttl = IPDEFTTL as uint8_t;
        ip.ip_p = IPPROTO_UDP_0 as uint8_t;
        if !src.is_null() {
    ...
    udp.uh_sum =
        if sum == 0xffff as libc::c_int as libc::c_uint { sum } else { !sum }
            as u16_0;
    rc = gettimeofday(&mut time, NULL_0 as *mut libc::c_void);
