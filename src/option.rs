...
//   { 'c', ARG_ONE, "<integer>", gettext_noop("Specify the size of the cache in entries (defaults to {})."), "$" },
//   { 'C', ARG_DUP, "<path>", gettext_noop("Specify configuration file (defaults to {})."), CONFFILE },
...
//   { 'g', ARG_ONE, "<groupname>", gettext_noop("Change to this group after startup (defaults to {})."), CHGRP },
...
//   { 'h', OPT_NO_HOSTS, NULL, gettext_noop("Do NOT load {} file."), HOSTSFILE },
//   { 'H', ARG_DUP, "<path>", gettext_noop("Specify a hosts file to be read in addition to {}."), HOSTSFILE },
...
//   { 'l', ARG_ONE, "<path>", gettext_noop("Specify where to store DHCP leases (defaults to {})."), LEASEFILE },
...
//   { 'n', OPT_NO_POLL, NULL, gettext_noop("Do NOT poll {} file, reload only on SIGHUP."), RESOLVFILE }, 
//   { 'N', OPT_NO_NEG, NULL, gettext_noop("Do NOT cache failed search results."), NULL },
//   { 'o', OPT_ORDER, NULL, gettext_noop("Use nameservers strictly in the order given in {}."), RESOLVFILE },
...
//   { 'P', ARG_ONE, "<integer>", gettext_noop("Maximum supported UDP packet size for EDNS.0 (defaults to {})."), "*" },
//   { 'q', ARG_DUP, NULL, gettext_noop("Log DNS queries."), NULL },
//   { 'Q', ARG_ONE, "<integer>", gettext_noop("Force the originating port for upstream DNS queries."), NULL },
//   { 'R', OPT_NO_RESOLV, NULL, gettext_noop("Do NOT read resolv.conf."), NULL },
//   { 'r', ARG_DUP, "<path>", gettext_noop("Specify path to resolv.conf (defaults to {})."), RESOLVFILE }, 
...
//   { 'u', ARG_ONE, "<username>", gettext_noop("Change to this user after startup. (defaults to {})."), CHUSER }, 
...
//   { 'x', ARG_ONE, "<path>", gettext_noop("Specify path of PID file (defaults to {})."), RUNFILE },
//   { 'X', ARG_ONE, "<integer>", gettext_noop("Specify maximum number of DHCP leases (defaults to {})."), "&" },
...
//   { 'Z', OPT_ETHERS, NULL, gettext_noop("Read DHCP  host information from {}."), ETHERSFILE },
...
//   { '0', ARG_ONE, "<integer>", gettext_noop("Maximum number of concurrent DNS queries. (defaults to {})"), "!" }, 
//   { LOPT_RELOAD, OPT_RELOAD, NULL, gettext_noop("Clear DNS cache when reloading {}."), RESOLVFILE },
...
//   { LOPT_TFTP_MAX, ARG_ONE, "<integer>", gettext_noop("Maximum number of concurrent TFTP transfers (defaults to {})."), "#" },
...
// pub fn unhide_metas(cp: &mut String)
// {
//   if (cp)
//     for(; *cp; cp++)
//       *cp = unhide_meta(*cp);
...
//  char *opt_string_alloc(const char *cp)
// {
//   char *ret = NULL;
//   len: usize;
...
//  char *split_chr(s: &mut String, char c)
// {
//   comma: &mut String, *p;
...
//  char *split(s: &mut String)
// {
//   return split_chr(s, ',');
// }

//  char *canonicalise_opt(s: &mut String)
// {
//   char *ret;
//   let mut nomem: i32;
...
#ifndef NO_ID
pub fn add_txt(name: &mut String, txt: &mut String, stat: i32)
{
  struct txt_record *r = opt_malloc(sizeof(struct txt_record));

  if (txt)
    {
      len: usize = strlen(txt);
...
pub fn do_usage()
{
  ...
  printf(format!("Usage: dnsmasq [options]\n\n"));
#ifndef HAVE_GETOPT_LONG
  printf(format!("Use short options only on the command line.\n"));

  printf(format!("Valid options are:\n"));
  ...
  for (i = 0; usage[i].opt != 0; i++)
	...
	sprintf(buff, "    ");
      ...
      sprintf(buff+4, "--{}{}{}", opts[j].name, eq, desc);
      printf("%-55.55s", buff);
      ...
      if (usage[i].arg)
	{
	  ...
	  for (j = 0; tab[j].handle; j++)
	    if (tab[j].handle == *(usage[i].arg))
	      sprintf(buff, "{}", tab[j].val);
	}
      printf(format!(usage[i].desc), buff);
      printf("\n");
    }
}
...
char *parse_server(arg: &mut String, union mysockaddr *addr, union mysockaddr *source_addr, interface: &mut String, int *flags)
{
  ...
  if (source) {
	...
	return format!("interface binding not supported");
 ...
 HAVE_SOCKADDR_SA_LEN
...
#if defined(SO_BINDTODEVICE)
	      if (interface_opt)
		return format!("interface can only be specified once");
	...
	return format!("bad interface name");
 ...
 struct server *add_rev4(addr: net::IpAddr, msize: i32)
{
  ...
  switch (msize)
    ...
    case 32:
      ...
      /* fall through */
    case 24:
      p += sprintf(p, "{}.", (a >> 8) & 0xff);
      /* fall through */
    case 16:
      p += sprintf(p, "{}.", (a >> 16) & 0xff);
      /* fall through */
    case 8:
      p += sprintf(p, "{}.", (a >> 24) & 0xff);
      break;
    default:
      free(serv.domain);
      free(serv);
 ...
 struct server *add_rev6(addr: &mut net::IpAddr, msize: i32)
{
  struct server *serv = opt_malloc(sizeof(struct server));
  char *p;
  let mut i: i32;
 ...
 int is_tag_prefix(arg: &mut String)
{
 ...
 char *set_prefix(arg: &mut String)
{
 ...
 struct dhcp_netid *dhcp_netid_create(const net: &mut String, struct dhcp_netid *next)
{
  let mut tt: dhcp_netid;
...
pub fn dhcp_netid_free(struct dhcp_netid *nid)
{
  while (nid)
    {
 ...
 * Moves arg to the end of tags. */
 struct dhcp_netid * dhcp_tags(char **arg)
{
...
pub fn dhcp_netid_list_free(struct dhcp_netid_list *netid)
{
  while (netid)
    {
...
pub fn dhcp_config_free(struct dhcp_config *config)
{
  if (config)
    {
...
pub fn dhcp_context_free(struct dhcp_context *ctx)
{
  if (ctx)
    {
...
pub fn dhcp_opt_free(struct dhcp_opt *opt)
{
 ...
 int parse_dhcp_opt(errstr: &mut String, arg: &mut String, flags: i32)
{
  ...
  char lenchar = 0, *cp;
  addrs: i32, digs, is_addr, is_addr6, is_hex, is_dec, is_string, dots;
  char *comma = NULL;
  u16 opt_len = 0;
  let mut is6: i32 = 0;
  let mut option_ok: i32 = 0;
  ...
  while (arg)
	...
	{
	  new.u.vendor_class = opt_string_alloc(arg+7);
  ...
  if (is6)
	...
	goto_err(format!("unsupported encapsulation for IPv6 option"));
  ...
  if (!option_ok)
    goto_err(format!("bad dhcp-option"));
  ...
  if (comma)
      ...
      if (opt_len & OT_ADDR_LIST)
	{
	  ...
	  if (!is6 && (!is_addr || dots == 0))
	    goto_err(format!("bad IP address"));

	   if (is6 && !is_addr6)
	     goto_err(format!("bad IPv6 address"));
	}
      /* or names */
      ...
      else if (is_addr && !is6)	
	{
	  ...
	  while (addrs--) 
		...
		goto_err(format!("bad IPv4 address"));
	      if (!slash)
		{
      ...
      else if (is_string)
	{
	  ...
	  if ((new.opt == OPTION_DOMAIN_SEARCH || new.opt == OPTION_SIP_SERVER) &&
	      !is6 && !(new.flags & (DHOPT_ENCAPSULATE | DHOPT_VENDOR | DHOPT_RFC3925)))
	    {
	      ...
	      while (arg && *arg)
		{
		  in: &mut String, *dom = NULL;
		  domlen: usize = 1;
		  /* Allow "." as an empty domain */
		  if (strcmp (arg, ".") != 0)
		    {
		      if (!(dom = canonicalise_opt(arg)))
			goto_err(format!("bad domain in dhcp-option"));
	  ...
	  else
	    {
	      ...
	      new.val = opt_string_alloc(comma);
  ...
  if (!is6 && 
    ...
    goto_err(format!("dhcp-option too long"));
  ...
  if (flags == DHOPT_MATCH)
    {
	  ...
	  !new.netid ||
	  new.netid.next)
	goto_err(format!("illegal dhcp-match"));
...
pub fn server_list_free(struct server *list)
{
  while (list)
    {
...
{      
	 ...
	 if (command_line)
	   {
	     /* command line */
	     if (rept == ARG_USED_CL)
	       ret_err(format!("illegal repeated flag"));
	     if (rept == ARG_ONE)
	 ...
	 else
	   {
	     ...
	     if (rept == ARG_USED_FILE)
	       ret_err(format!("illegal repeated keyword"));
  ...
  switch (option)
	...
	if (!(dir_stream = opendir(directory)))
	  die(format!("cannot access directory {}: {}"), directory, EC_FILE);
	
	while ((ent = readdir(dir_stream)))
	  {
	    len: usize = strlen(ent.d_name);
	    struct stat buf;
	    ...
	    if (stat(path, &buf) == -1)
	      die(format!("cannot access {}: {}"), path, EC_FILE);
	...
	{	  
 __ANDROID__
	  ret_err(format!("setting log facility is not possible under Android"));
	  ...
	  if (facilitynames[i].c_name)
	    ...
	    ret_err(format!("bad log facility"));
	...
	if ((comma = split(arg)))
	  {
	    ...
	    if ((prefstr = split(comma)) && !atoi_check16(prefstr, &pref))
	      ret_err(format!("bad MX preference"));
	  }
	
	if (!(name = canonicalise_opt(arg)) || 
	    (comma && !(target = canonicalise_opt(comma))))
	  ret_err(format!("bad MX name"));
...
#  endif
			  ...
			  if (arg)
			      ...
			      if (option != 's')
				{
				  if (!(new.prefix = canonicalise_opt(arg)) ||
				    ...
				    ret_err_free(format!("bad prefix"), new);
				}
 ...
 HAVE_LOOP
    ...
    case LOPT_IPSET: /* --ipset */
#ifndef HAVE_IPSET
      ret_err(format!("recompile with HAVE_IPSET defined to enable ipset directives"));
      break;
	 ...
	 sets = sets_pos = opt_malloc(sizeof * size);
    ...
    case LOPT_BRIDGE:   /* --bridge-interface */
      {
	let mut new: dhcp_bridge;

	if (!(comma = split(arg)) || strlen(arg) > IF_NAMESIZE - 1 )
	  ret_err(format!("bad bridge-interface"));
	...
	if (!(comma = split(arg)))
	  ...
	  snerr:
	    free(new);
	    ret_err(format!("bad shared-network"));
	  }
    ...
    case 'F':  /* --dhcp-range */
      {
	k: i32, leasepos = 2;
	cp: &mut String, *a[8] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };
	...
	while(1)
	  {
		...
		if (is_tag_prefix(arg))
		...
		else
		  {
			...
			ret_err(format!("only one tag allowed"));
		      }
		    else
	...
	if (inet_pton(AF_INET, a[0], &new.start))
	  {
		...
		if (!is_same_net(new.start, new.end, new.netmask))
		  {
		    dhcp_context_free(new);
		    ret_err(format!("inconsistent DHCP range"));
		  }
	...
	else if (inet_pton(AF_INET6, a[0], &new.start6))
	  {
	    ...
	    for (leasepos = 1; leasepos < k; leasepos++)
	      {
		if (strcmp(a[leasepos], "") == 0)
		  new.flags |= CONTEXT_ | CONTEXT_DHCP;
		...
		if (new.flags & CONTEXT_RA)
		  err=(format!("prefix length must be exactly 64 for RA subnets"));
		else if (new.flags & CONTEXT_TEMPLATE)
		  err=(format!("prefix length must be exactly 64 for subnet constructors"));
	      }
	    else if (new.prefix < 64)
	      err=(format!("prefix length must be at least 64"));
	    
	    if (!err && !is_same_net6(&new.start6, &new.end6, new.prefix))
	      err=(format!("inconsistent DHCPv6 range"));
		...
		if (!is_same_net6(&zero, &new.start6, new.prefix))
		  {
		    dhcp_context_free(new);
		    ret_err(format!("prefix must be zero with \"constructor:\" argument"));
		  }
	      }
		...
		if (strlen(a[leasepos]) > 0)
		  {
		    ...
		    if (*cp || (leasepos+1 < k))
		      ret_err_free(format!("bad dhcp-range"), new);
	...
	while (arg)
	  {
		...
		if ((arg[0] == 'i' || arg[0] == 'I') &&
		    ...
		    arg[2] == ':')
		  {
			...
			arg += 3; /* dump id: */
			if (strchr(arg, ':'))
			  len = parse_hex(arg, arg, -1, NULL, NULL);
			else
			  {
			    unhide_metas(arg);
			    len =  strlen(arg);
			...
			if (len == -1)
			  {
			    ...
			    ret_err(format!("bad hex constant"));
			  }
		...
		/* dhcp-host has strange backwards-compat needs. */
		...
		else if (arg[0] == '[' && arg[strlen(arg)-1] == ']')
		  {
		    ...
		    if (!inet_pton(AF_INET6, arg, &in6))
		      {
			dhcp_config_free(new);
			ret_err(format!("bad IPv6 address"));
		      }
			...
			if (!atoi_check(pref, &new_addr.prefixlen) ||
			  ...
			  {
			    dhcp_config_free(new);
			    ret_err(format!("bad IPv6 prefix"));
			  }
		...
		for (configs = daemon.dhcp_conf; configs; configs = configs.next) 
		  if ((configs.flags & CONFIG_ADDR) && configs.addr.s_addr == in.s_addr)
		    {
		      sprintf(errstr, format!("duplicate dhcp-host IP address {}"),  inet_ntoa(in));
		      return 0;
		    }	      
	      }
	    else
		...
		if (*cp)
		  {
			...
			if (!(new.hostname = canonicalise_opt(arg)) ||
			  ...
			  {
			    dhcp_config_free(new);
			    ret_err(format!("bad DHCP host name"));
			  }
	...
	if (!new.set)
	  {
	    ...
	    ret_err_free(format!("bad tag-if"), new);
	  }
	...
	if (!arg)
	  ...
	  {
	    dhcp_file: &mut String, *dhcp_sname = NULL, *tftp_sname = NULL;
	    dhcp_next_server: net::IpAddr;
	    let mut new: dhcp_boot;
	    comma = split(arg);
    ...
    case LOPT_SUBSCR:   /* --dhcp-subscrid */
	 ...
	 if (option == 'U' || option == 'j' || *p || !dig)
	   ...
	   {
	     new.len = parse_hex(comma, comma, strlen(comma), NULL, NULL);
	     new.data = opt_malloc(new.len);
	     memcpy(new.data, comma, new.len);
	   }
      ...
      else
	{
	  ...
	  if (!atoi_check16(arg, &daemon.dhcp_server_port) || 
	    ...
	    ret_err(format!("invalid port number"));
	  if (!comma)
	...
	if (!(inet_pton(AF_INET, arg, &new.addr) > 0))
	  ret_err_free(format!("bad dhcp-proxy address"), new);
	...
	if (inet_pton(AF_INET, arg, &new.local) && inet_pton(AF_INET, comma, &new.server))
	  ...
	  {
	    free(new.interface);
	    ret_err_free(format!("Bad dhcp-relay"), new);
	  }
...
err:
	       free(new.name);
	       ret_err_free(format!("bad RA-params"), new);
             }
	...
	ret_err(format!("bad DUID"));
      else
	{
	  daemon.duid_config_len = parse_hex(comma,comma, strlen(comma), NULL, NULL);
    ...
    case 'V':  /* --alias */
      {
	dash: &mut String, *a[3] = { NULL, NULL, NULL };
	let mut k: i32 = 0;
	...
	if ((k < 2) || 
	  ...
	  ret_err(format!("missing address in alias"));
	
	if (dash && 
	    (!(inet_pton(AF_INET, dash, &new.end) > 0) ||
	     !is_same_net(new.in, new.end, new.mask) ||
	     ntohl(new.in.s_addr) > ntohl(new.end.s_addr)))
	  ret_err_free(format!("invalid alias range"), new);
	...
	if (!comma || !(domain = canonicalise_opt(arg)))
	  ret_err(format!("bad interface name"));
	...
	*up = new;
	...
	while (arg != last)
	  {
	      ...
	      if (hostname_isequal(new.alias, alias))
		{
		  free(target);
		  free(alias);
		  ret_err(format!("duplicate CNAME"));
		}
    ...
    case LOPT_PTR:  /* --ptr-record */
      {
	let mut new: ptr_record;
	dom: &mut String, *target = NULL;
	...
	if (k < 6 || 
	    ...
	    free(name);
	    free(replace);
	    ret_err(format!("bad NAPTR record"));
          }
	else
	  {
	...
	if (!atoi_check(comma, &class) || 
	    ...
	    (data && (len = parse_hex(data, data, -1, NULL, NULL)) == -1U))
          {
            free(new.name);
	    ret_err_free(format!("bad RR record"), new);
          }
	...
	if (!(new.name = canonicalise_opt(arg)))
	  ret_err_free(format!("bad TXT record"), new);
	...
	*cnt = 0;
	...
	if (comma)
	  {
	    ...
	    if (!(target = canonicalise_opt(arg)))
	      ret_err_free(format!("bad SRV target"), name);
		...
		if (!atoi_check16(arg, &port))
                  {
                    free(name);
		    ret_err_free(format!("invalid port number"), target);
                  }
		...
		if (comma)
		  {
		    ...
		    if (!atoi_check16(arg, &priority))
                      {
                        free(name);
		        ret_err_free(format!("invalid priority"), target);
		      }
		    if (comma && !atoi_check16(comma, &weight))
                      {
                        free(name);
		        ret_err_free(format!("invalid weight"), target);
                      }
		  }
	      }
	  }
    ...
    case LOPT_HOST_REC: /* --host-record */
      {
	let mut new: host_record;

	if (!arg || !(comma = split(arg)))
	  ret_err(format!("Bad host-record"));
	...
	while (arg)
	  {
		...
		if (!canon)
		    ...
		    ret_err_free(format!("Bad name in host-record"), new);
                  }
	...
	{
	  if (strcmp(arg, "no") == 0)
	    set_daemon.opt_dnssec_ign_ns;
	  else
	    ret_err(format!("bad value for dnssec-check-unsigned"));
	}
      break;
	...
	struct ds_config *new = opt_malloc(sizeof(struct ds_config));
      	cp: &mut String, *cp1, *keyhex, *digest, *algo = NULL;
	let mut len: i32;
	...
	if ((comma = split(arg)) && (algo = split(comma)))
	  {
	    ...
	    !(new.name = canonicalise_opt(arg)))
	  ret_err_free(format!("bad trust anchor"), new);
	...
	for (cp = keyhex; *cp; )
	  if (isspace(*cp))
	    ...
	    cp +=1;
	if ((new.digestlen = parse_hex(keyhex, new.digest, len, NULL, NULL)) == -1)
	  {
	    free(new.name);
	    ret_err_free(format!("bad HEX in trust anchor"), new);
	  }
    ...
    default:
      ret_err(format!("unsupported option (check that dnsmasq was compiled with DHCP/TFTP/DNSSEC/DBus support)"));
...
pub fn read_file(file: &mut String, FILE *f, hard_opt: i32)	
{
  ...
  while (fgets(buff, MAXDNAME, f))
      ...
      if (option == 0)
	{
	  ...
	  if (!option)
	    errmess = format!("bad option");
	  else if (opts[i].has_arg == 0 && arg)
	    errmess = format!("extraneous parameter");
	  else if (opts[i].has_arg == 1 && !arg)
	    errmess = format!("missing parameter");
	  else if (hard_opt == LOPT_REV_SERV && option != 'S' && option != LOPT_REV_SERV)
	    errmess = format!("illegal option");
	}
	...
	strcpy(daemon.namebuff, errmess);
      ...
      if (errmess || !one_opt(option, arg, daemon.namebuff, format!("error"), 0, hard_opt == LOPT_REV_SERV))
	{
	  sprintf(daemon.namebuff + strlen(daemon.namebuff), format!(" at line {} of {}"), lineno, file);
	  if (hard_opt != 0)
	    my_syslog(LOG_ERR, "{}", daemon.namebuff);
	  else
	    die("{}", daemon.namebuff, EC_BADCONF);
	}
    }
	...
	if (stat(ah.fname, &buf) != -1 && S_ISDIR(buf.st_mode))
	  {
	    ...
	    if (!(dir_stream = opendir(ah.fname)))
	      my_syslog(LOG_ERR, format!("cannot access directory {}: {}"), 
...
void read_servers_file()
{
  ...
  if (!(f = fopen(daemon.servers_file, "r")))
    {
       my_syslog(LOG_ERR, format!("cannot read {}: {}"), daemon.servers_file, strerror(errno));
       return;
    }
	...
	if (!(hf.flags & AH_INACTIVE))
	  {
	    if (one_file(hf.fname, LOPT_BANK))  
	      my_syslog(MS_DHCP | LOG_INFO, format!("read {}"), hf.fname);
	  }
    }
...
void read_opts(argc: i32, char **argv, compile_opts: &mut String)
{
  ...
  option: i32, testmode = 0;
  arg: &mut String, *conffile = NULL;
 ...
 HAVE_GETOPT_LONG
      ...
      if (option == -1)
	{
	  for (; optind < argc; optind++)
	    {
	      unsigned char *c = argv[optind];
	      for (; *c != 0; c++)
		if (!isspace(*c))
		  die(format!("junk found in command line"), NULL, EC_BADCONF);
	    }
	  break;
	}
	...
	{
	  printf(format!("Dnsmasq version {}  {}\n"), VERSION, COPYRIGHT);
	  printf(format!("Compile time options: {}\n\n"), compile_opts); 
	  printf(format!("This software comes with ABSOLUTELY NO WARRANTY.\n"));
	  printf(format!("Dnsmasq is free software, and you are welcome to redistribute it\n"));
	  printf(format!("under the terms of the GNU General Public License, version 2 or 3.\n"));
          exit(0);
        }
      else if (option == 'C')
	{
	...
	{
 HAVE_GETOPT_LONG
	  if (!one_opt(option, arg, daemon.namebuff, format!("try --help"), 1, 0))
 
	    if (!one_opt(option, arg, daemon.namebuff, format!("try -w"), 1, 0)) 
  
	    die(format!("bad command line options: {}"), daemon.namebuff, EC_BADCONF);
	}
    }
...
pub const TESTLOOP: u32 = 2;      
      ...
      for (cn = daemon.cnames; cn; cn = cn.next)
	{
	  for (cn2 = cn.targetp; cn2; cn2 = cn2.targetp)
		...
		die(format!("CNAME loop involving {}"), cn.alias, EC_BADCONF);
  ...
  if (option_bool(OPT_LOCALMX) || daemon.mxnames || daemon.mxtarget)
	...
	die(format!("cannot get host-name: {}"), NULL, EC_MISC);
  ...
  if (!daemon.opt_no_resolv &&
      ...
      daemon.opt_no_poll)
    die(format!("only one resolv.conf file allowed in no-poll mode."), NULL, EC_BADCONF);
  ...
  if (option_bool(OPT_RESOLV_DOMAIN))
    {
	  ...
	  !daemon.resolv_files || 
	  (daemon.resolv_files).next)
	die(format!("must have exactly one resolv.conf to read domain from."), NULL, EC_BADCONF);
      
      if (!(f = fopen((daemon.resolv_files).name, "r")))
	die(format!("failed to read {}: {}"), (daemon.resolv_files).name, EC_FILE);
	...
	die(format!("no search directive found in {}"), (daemon.resolv_files).name, EC_MISC);
    }

  if (daemon.domain_suffix)
    {
       /* add domain for any srv record without one. */
      let mut srv: mx_srv_record;
    ...
    die(format!("there must be a default domain when --dhcp-fqdn is set"), NULL, EC_BADCONF);
