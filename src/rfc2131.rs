...
{
  unsigned char *opt, *clid = nullptr;
  struct dhcp_lease *ltmp, *lease = nullptr;
  ...
  char *hostname = nullptr, *offer_hostname = nullptr, *client_hostname = nullptr, *domain = nullptr;
  int hostname_auth = 0, borken_opt = 0;
  unsigned char *req_options = nullptr;
  ...
  unsigned char *agent_id = nullptr, *uuid = nullptr;
  unsigned char *emac = nullptr;
  ...
  */
  ...
  config = find_config(daemon->dhcp_conf, context, clid, clid_len, 
		       mess->chaddr, mess->hlen, mess->htype, nullptr);
  ...
  else if (find_config(daemon->dhcp_conf, nullptr, clid, clid_len,
		       mess->chaddr, mess->hlen, mess->htype, nullptr))
    {
  ...
  if (mess_type == 0 && !pxe)
    {
      ...
      struct in_addr *logaddr = nullptr;
      ...
      if (!message)
	{
	  ...
	  if (have_config(config, CONFIG_ADDR))
	    ...
	    {
	      if (!(lease = lease_find_by_client(mess->chaddr, mess->hlen, mess->htype, nullptr, 0)) ||
	  ...
	  if (!message)
	    {
	      ...
	      lease_set_hwaddr(lease, mess->chaddr, nullptr, mess->hlen, mess->htype, 0, now, 1);
	      if (hostname)
		lease_set_hostname(lease, hostname, 1, get_domain(lease->addr), domain); 
	      ...
	      do_options(context, mess, end, nullptr, hostname, get_domain(mess->yiaddr),
			 netid, subnet_addr, 0, 0, -1, nullptr, vendor_class_len, now, 0xffffffff, 0);
	    }
	}
      
      daemon->metrics[METRIC_BOOTP]++;
      log_packet("BOOTP", logaddr, mess->chaddr, mess->hlen, iface_name, nullptr, message, mess->xid);
	  ...
	  if (!config)
	    {
		 ...
		 to avoid impersonation by name. */
	      struct dhcp_config *new = find_config(daemon->dhcp_conf, context, nullptr, 0,
  ...
  if (daemon->enable_pxe && 
	  ...
	  (opt = option_find1(option_ptr(opt, 0), option_ptr(opt, option_len(opt)), SUBOPT_PXE_BOOT_ITEM, 4)))
	{
	  ...
	  log_packet("PXE", &mess->yiaddr, emac, emac_len, iface_name, (char *)mess->file, nullptr, mess->xid);
	  log_tags(tagif_netid, ntohl(mess->xid));
      ...
      if ((opt = option_find(mess, sz, OPTION_ARCH, 2)))
	{
	  ...
	  if ((mess_type == DHCPDISCOVER || (pxe && mess_type == DHCPREQUEST)))
	      ...
	      if (tmp)
		{
		  ...
		  log_packet("PXE", nullptr, emac, emac_len, iface_name, ignore ? "proxy-ignored" : "proxy", nullptr, mess->xid);
		  log_tags(tagif_netid, ntohl(mess->xid));
		  if (!ignore)
  ...
  switch (mess_type)
	...
	return 0;
      ...
      log_packet("DHCPDECLINE", option_ptr(opt, 0), emac, emac_len, iface_name, nullptr, daemon->dhcp_buff, mess->xid);
	...
	message = _("unknown lease");
      ...
      log_packet("DHCPRELEASE", &mess->ciaddr, emac, emac_len, iface_name, nullptr, message, mess->xid);
      ...
      else 
	{
      ...
      log_packet("DHCPDISCOVER", opt ? option_ptr(opt, 0) : nullptr, emac, emac_len, iface_name, nullptr, message, mess->xid);
      ...
      if (option_bool(OPT_RAPID_COMMIT) && option_find(mess, sz, OPTION_RAPID_COMMIT, 0))
	{
      ...
      log_packet("DHCPOFFER" , &mess->yiaddr, emac, emac_len, iface_name, nullptr, nullptr, mess->xid);
      ...
      else
	{
      ...
      log_packet("DHCPREQUEST", &mess->yiaddr, emac, emac_len, iface_name, nullptr, nullptr, mess->xid);
      ...
      if (message)
	{
	  ...
	  log_packet(rapid_commit ? "NOANSWER" : "DHCPNAK", &mess->yiaddr, emac, emac_len, iface_name, nullptr, message, mess->xid);
      ...
      else
	{
	  ...
	  if (do_classes)
	      ...
	      if (daemon->lease_change_command)
		{
		  ...
		  add_extradata_opt(lease, _class);
		  ...
		  if ((opt = option_find(mess, sz, OPTION_AGENT_ID, 1)))
		    ...
		    {
		      add_extradata_opt(lease, nullptr);
		      add_extradata_opt(lease, nullptr);
		      add_extradata_opt(lease, nullptr);
		    }
	  ...
	  log_packet("DHCPACK", &mess->yiaddr, emac, emac_len, iface_name, hostname, nullptr, mess->xid);
	...
	message = _("ignored");
      ...
      log_packet("DHCPINFORM", &mess->ciaddr, emac, emac_len, iface_name, message, nullptr, mess->xid);
      ...
      if (context && context->netid.net)
	{
      ...
      daemon->metrics[METRIC_DHCPACK]++;
      log_packet("DHCPACK", &mess->ciaddr, emac, emac_len, iface_name, hostname, nullptr, mess->xid);
...
{
  if (!opt)
    lease_add_extradata(lease, nullptr, 0, 0);
  else
