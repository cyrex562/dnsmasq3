...
pub fn ra_init(mut now: time::Instant) {
    ...
    if daemon.doing_ra != 0 {
        filter.icmp6_filt[(133 >> 5)] &= !((1) << (133 & 31));
        if !context.is_null() {
            filter.icmp6_filt[(129 >> 5)] &= !((1) << (129 & 31))
        }
    }
    fd =
        socket(10, SOCK_RAW,
    ...
    if fd == -(1) ||
        ...
        panic!("cannot create ICMPv6 socket: {}", 0, );
    }
    daemon.icmp6fd = fd;
    if daemon.doing_ra != 0 {
...
fn send_ra_alias(mut now: time::Instant, mut iface: i32,
    ...
    reset_counter();
    ra =
        expand(::std::mem::size_of::<ra_packet>())
    ra_packet;
    ...
    if mtu == 0 {
        ...
        sprintf(daemon.namebuff,
                "/proc/sys/net/ipv6/conf/%s/mtu" *const libc
        ::c_char,
                        ...
                        while i < opt_cfg.len {
                            if ({
                                ...
                                if parm.glob_pref_time !=
                                    0 {
                                    put_opt6(&mut parm.link_global & mut In6Addr
                                    Vec < u8 >,
                                    16 );
                                }
                            ...
                            0
                            {
                                if parm.link_pref_time !=
                                    0 {
                                    put_opt6(&mut parm.link_local & mut In6Addr
                                    Vec < u8 >,
                                    16 );
                                }
                            } else {
