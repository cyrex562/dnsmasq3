...
//#endif
    ...
    die (_("cannot create ICMPv6 socket: %s"), nullptr, EC_BADNET);
...
void icmp6_packet(time_t now)
{
  ...
  if (!iface_check(AF_LOCAL, nullptr, interface, nullptr))
    return;
  ...
  if (packet[0] == ICMP6_ECHO_REPLY)
		...
		if (wildcard_matchn(alias->iface, interface, IF_NAMESIZE))
		  {
		    /* Send an RA on if_index with information from
		       bridge_index. */
		    send_ra_alias(now, bridge_index, bridge->iface, nullptr, if_index);
		    break;
		  }
	      if (alias)
		break;
	...
	send_ra(now, if_index, interface, !IN6_IS_ADDR_UNSPECIFIED(&from.sin6_addr) ? &from.sin6_addr : nullptr);
    }
}
...
//#ifdef HAVE_LINUX_NETWORK
  ...
  if (mtu == 0)
    {
      char *mtu_name = ra_param ? ra_param->mtu_name : nullptr;
...
//#endif
  ...
  (void)option_filter(parm.tags, nullptr, daemon->dhcp_opts6);
...
time_t periodic_ra(time_t now)
{
  ...
  while (1)
	  ...
	  iface_check(AF_LOCAL, nullptr, param.name, nullptr))
	{
	  ...
	  if (!tmp)
            {
              send_ra(now, param.iface, param.name, nullptr);
