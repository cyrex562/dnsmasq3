...
struct ra_param {
  now: time::Instant;
  ind: i32, managed, other, first, adv_router;
  char *if_name;
  let mut tags: dhcp_netid;
  struct in6_addr link_local, link_global, ula;
  unsigned glob_pref_time: i32, link_pref_time, ula_pref_time, adv_interval, prio;
  let mut found_context: dhcp_context;
};

struct search_param {
  now: time::Instant; let mut iface: i32;
  char name[IF_NAMESIZE+1];
};

struct alias_param {
  let mut iface: i32;
  let mut bridge: dhcp_bridge;
  let mut num_alias_ifs: i32;
  let mut max_alias_ifs: i32;
  int *alias_ifs;
};

//  let mut hop_limit: i32;

pub fn ra_init(now: time::Instant)
{
  ...
  let mut val: i32 = 255; /* radvd uses this value */
...
pub fn icmp6_packet(now: time::Instant)
{
  ...
  if (packet[0] == ICMP6_ECHO_REPLY)
	...
	my_syslog(MS_DHCP | LOG_INFO, "RTR-SOLICIT({}) {}", interface, mac);
      ...
      for (bridge = daemon.bridges; bridge; bridge = bridge.next)
        {
          bridge_index: i32 = if_nametoindex(bridge.iface);
          if (bridge_index)
	    {
...
pub fn send_ra_alias(now: time::Instant, iface: i32, iface_name: &mut String, dest: &mut net::IpAddr, send_iface: i32)
{
  let mut ra: ra_packet;
  ...
  done_dns: i32 = 0, old_prefix = 0, mtu = 0;
  let mut min_pref_time: u32;
  ...
  for (up = &daemon.dhcp6, context = daemon.dhcp6; context; context = tmp)
	...
	{
	  unsigned old: i32 = difftime(now, context.address_lost_time);
	  ...
	  if (old > context.saved_valid)
	      ...
	      if ((opt = expand(sizeof(struct prefix_opt))))
		{
		  ...
		  if (!option_bool(OPT_QUIET_RA))
		    my_syslog(MS_DHCP | LOG_INFO, "RTR-ADVERT({}) {} old prefix", iface_name, daemon.addrbuff); 		    
		}
  ...
  if (mtu == 0)
    {
      ...
      sprintf(daemon.namebuff, "/proc/sys/net/ipv6/conf/{}/mtu", mtu_name ? mtu_name : iface_name);
  ...
  ()option_filter(parm.tags, NULL, daemon.dhcp_opts6);
 ...
 HAVE_SOCKADDR_SA_LEN
  ...
  while (retry_send(sendto(daemon.icmp6fd, daemon.outpacket.iov_base, 
			   save_counter(-1), 0, &addr, 
			   sizeof(addr))));
  
}

pub fn send_ra(now: time::Instant, iface: i32, iface_name: &mut String, dest: &mut net::IpAddr)
{
 ...
 add_prefixes: i32(local: &mut net::IpAddr,  prefix: i32,
			scope: i32, if_index: i32, flags: i32, 
			unsigned preferred: i32, unsigned valid: i32, vparam: Vec<u8>)
{
	       ...
	       !IN6_IS_ADDR_MULTICAST(local))
	{
	  ...
	  if (real_prefix != 0)
	      ...
	      if ((opt = expand(sizeof(struct prefix_opt))))
		{
		  ...
		  inet_ntop(AF_INET6, local, daemon.addrbuff, ADDRSTRLEN);
		  if (!option_bool(OPT_QUIET_RA))
		    my_syslog(MS_DHCP | LOG_INFO, "RTR-ADVERT({}) {}", param.if_name, daemon.addrbuff); 		    
		}
	    }
	}
    }          
 ...
 add_lla: i32(index: i32, unsigned type: i32, mac: &mut String, maclen: usize, parm: Vec<u8>)
{
  ()type;
...
periodic_ra: time::Instant(now: time::Instant)
{
                ...
                if (if_nametoindex(aparam.bridge.iface) == param.iface)
                  {
                    ...
                    my_syslog(MS_DHCP | LOG_INFO, "RTR-ADVERT({}) {} => {} alias(es)",
                    ...
                    if (aparam.alias_ifs)
                      {
                        ...
                        for (; aparam.num_alias_ifs; aparam.num_alias_ifs--)
                          {
                            my_syslog(MS_DHCP | LOG_INFO, "RTR-ADVERT({}) {} => i/f {}",
 ...
 send_ra_to_aliases: i32(index: i32, unsigned type: i32, mac: &mut String, maclen: usize, parm: Vec<u8>)
{
  struct alias_param *aparam = (struct alias_param *)parm;
  char ifrn_name[IFNAMSIZ];
  let mut alias: dhcp_bridge;
 ...
 iface_search: i32(local: &mut net::IpAddr,  prefix: i32,
			scope: i32, if_index: i32, flags: i32, 
...
pub fn new_timeout(struct dhcp_context *context, iface_name: &mut String, now: &time::Instant)
{
  if (difftime(now, context.ra_short_period_start) < 60.0)
  ...
  else
    {
      ...
      unsigned adv_interval: i32 = calc_interval(find_iface_param(iface_name));
      context.ra_time = now + (3 * adv_interval)/4 + ((adv_interval *rand16()) >> 18);
    }
}

 struct ra_interface *find_iface_param(iface: &mut String)
{
  let mut ra: ra_interface;
 ...
 unsigned calc_interval: i32(struct ra_interface *ra)
{
 ...
 unsigned calc_lifetime: i32(struct ra_interface *ra)
{
  lifetime: i32, interval = calc_interval(ra);
 ...
 unsigned calc_prio: i32(struct ra_interface *ra)
{
  if (ra)
    return ra.prio;
