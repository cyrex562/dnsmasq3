...
static void rehash(int size)
{
  ...
  if (!hash_table)
    _new = (crec*)safe_malloc(new_size * sizeof(struct crec *));
  else if (new_size <= hash_size || !(_new = (crec*)whine_malloc(new_size * sizeof(struct crec *))))
    return;
