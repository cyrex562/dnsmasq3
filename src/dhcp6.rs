...
static int
complete_context6(struct in6_addr* local,
        int prefix,
        int scope,
        int if_index,
        int flags,
...
static int
make_duid1(int index, unsigned int type, char* mac, size_t maclen, void* parm);
...
//#endif
        ...
        if (rc==-1) {
            die(_("failed to set SO_REUSE{ADDR|PORT} on DHCPv6 socket: %s"),
                    nullptr,
                    EC_BADNET);
        }
    }
...
//#endif
    ...
    if (bind(fd, (struct sockaddr*) &saddr, sizeof(struct sockaddr_in6))) {
        die(_("failed to bind DHCPv6 server socket: %s"), nullptr, EC_BADNET);
    }
...
dhcp6_packet(time_t now)
{
    ...
    if ((port = relay_reply6(&from, sz, ifr.ifr_name))==0) {
        ...
        for (bridge = daemon->bridges; bridge; bridge = bridge->next) {
            for (alias = bridge->alias; alias; alias = alias->next) {
                if (wildcard_matchn(alias->iface.c_str(), ifr.ifr_name, IF_NAMESIZE)) {
                    parm.ind = if_nametoindex(bridge->iface.c_str());
                    if (!parm.ind) {
                        spdlog::warn("unknown interface {} in bridge-interface",
                                bridge->iface.c_str());
                        return;
                    }
                    break;
    ...
    if (port!=0) {
        ...
        while (retry_send(sendto(daemon->dhcp6fd,
        ...
        void* vparam)
{
    ...
    if (if_index==param->ind) {
        ...
        if (!IN6_IS_ADDR_LOOPBACK(local) && !IN6_IS_ADDR_LINKLOCAL(local)
            ...
            for (context = daemon->dhcp6; context; context = context->next) {
                if ((context->flags & CONTEXT_DHCP)
                    ...
                    if (context->current==context) {
                        ...
                        for (up = &param->current, tmp = param->current;
                             tmp;
                             tmp = tmp->current) {
        ...
        u64 addr)
{
    ...
    for (config = configs; config; config = config->next) {
        if ((config->flags & CONFIG_ADDR6) && is_same_net6(&config->addr6, net, prefix)
        ...
        for (c = context; c; c = c->current) {
            if (c->flags & (CONTEXT_DEPRECATE | CONTEXT_STATIC | CONTEXT_RA_STATELESS
                ...
                do {
                    ...
                    if (!d && !lease6_find_by_addr(&c->start6, c->prefix, addr)
                            && !config_find_by_address6(daemon->dhcp_conf,
                                    &c->start6,
                                    c->prefix,
                                    addr)) {
                        *ans = c->start6;
        ...
        int plain_range)
{
    ...
    for (tmp = context; tmp; tmp = tmp->current) {
        ...
        if (!(tmp->flags & (CONTEXT_STATIC | CONTEXT_RA_STATELESS))
                ...
                && is_same_net6(&tmp->end6, taddr, tmp->prefix) && addr>=start
...
//#endif
        ...
        if (!daemon->duid) {
            die("Cannot create DHCPv6 server DUID: %s", nullptr, EC_MISC);
        }
    }
}
...
static int
construct_worker(struct in6_addr* local,
        int prefix,
        int scope,
        int if_index,
        int flags,
        ...
        void* vparam)
{
    ...
    if (!(template->flags & (CONTEXT_TEMPLATE | CONTEXT_CONSTRUCTED)))
    {
        ...
        template->prefix))
        {
            ...
            if (template->if_index!=if_index || !IN6_ARE_ADDR_EQUAL(&
                    template->local6, local))
            {
                ra_start_unsolicited(param->now,
                template);
                param->newone = 1;
            }
    ...
    {
        start6 = *local;
        setaddr6part(&start6, addr6part(&
        template->start6));
        end6 = *local;
        setaddr6part(&end6, addr6part(&
        template->end6));
