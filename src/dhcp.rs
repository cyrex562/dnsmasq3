...
// 			      struct in_addr netmask, struct in_addr broadcast, void *vparam);
// static int relay_upstream4(struct DhcpRelay *relay, struct dhcp_packet *mess, size_t sz, int iface_index);
// static struct DhcpRelay *relay_reply4(struct dhcp_packet *mess, char *arrival_interface);

pub make_fd(port: i32) -> i32
{
...
//#endif
  ...
  if (fd == -1)
    die (_("cannot create DHCP socket: %s"), nullptr, EC_BADNET);
...
//#endif
    ...
    die(_("failed to set options on DHCP socket: %s"), nullptr, EC_BADNET);
...
//#endif
	...
	rc = setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &oneopt, sizeof(oneopt));
      ...
      if (rc == -1)
	die(_("failed to set SO_REUSE{ADDR|PORT} on DHCP socket: %s"), nullptr, EC_BADNET);
    }
...
//#endif
  ...
  if (bind(fd, (struct sockaddr *)&saddr, sizeof(struct sockaddr_in)))
    die(_("failed to bind DHCP server socket: %s"), nullptr, EC_BADNET);
...
//#if defined(HAVE_BSD_NETWORK)
  ...
  if (option_bool(OPT_NO_PING))
    ...
    die(_("cannot create ICMP raw socket: %s."), nullptr, EC_BADNET);
...
//#endif 
	...
	{
	  if (iface_check(AF_INET, nullptr, ifr.ifr_name, nullptr))
	...
	relay->current = relay;
      ...
      parm.current = nullptr;
      parm.relay = nullptr;
      parm.relay_local.s_addr = 0;
      parm.ind = iface_index;
      
      if (!iface_check(AF_INET, (struct all_addr *)&iface_addr, ifr.ifr_name, nullptr))
	{
	  ...
	  complete_context(match.addr, iface_index, nullptr, match.netmask, match.broadcast, &parm);
	}    
	...
	return;
      ...
      lease_prune(nullptr, now); /* lose any expired leases */
...
struct ping_result *do_icmp_ping(time_t now, struct in_addr addr, unsigned int hash, int loopback)
{
  ...
  struct ping_result *r, *victim = nullptr;
...
void dhcp_read_ethers(void)
{
  ...
  while (fgets(buff, MAXDNAME, f))
	...
	*ip = 0;
      if (!*ip || parse_hex(buff, hwaddr, ETHER_ADDR_LEN, nullptr, nullptr) != ETHER_ADDR_LEN)
	{
	...
	{ 
	  ...
	  if (!config)
		...
		continue;
	      ...
	      config->hwaddr = nullptr;
	      config->domain = nullptr;
	      config->netid = nullptr;
...
char *host_from_dns(struct in_addr addr)
{
  ...
  if (daemon->port == 0)
    return nullptr; /* DNS disabled. */
  
  lookup = cache_find_by_addr(nullptr, (struct all_addr *)&addr, 0, F_IPV4);
  ...
  for (relay = daemon->relay4; relay; relay = relay->next)
      ...
      if (mess->giaddr.s_addr == relay->local.addr.addr4.s_addr)
	{
	  if (!relay->interface || wildcard_match(relay->interface, arrival_interface))
	    return relay->iface_index != 0 ? relay : nullptr;
	}
    }
