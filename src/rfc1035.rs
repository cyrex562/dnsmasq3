...
pub fn extract_name(mut header: &DnsHeader,
    ...
    if is_extract != 0 { cp = 0 }
    loop  {
        let mut label_type: u32 = 0;
        if !((p.wrapping_offset_from(header)  + 1)
                 <= plen) {
            return 0
        }
        let fresh6 = p;
        ...
        if label_type == 0xc0 {
            /* pointer */
            if !((p.wrapping_offset_from(header)                i32 + 1)               size_t <= plen) {
                return 0
            }
            /* get offset */
            l = (l & 0x3f) << 8;
            ...
            /* label_type = 0 -> label. */
            namelen =
                namelen.wrapping_add(l.wrapping_add(1     libc::c_uint)); /* include period */
            if namelen >= 1025 {
                return 0
            }
            if !((p.wrapping_offset_from(header)                i32 + l)  <= plen) {
                return 0
            }
            j = 0;
            while j < l {
                if is_extract != 0 {
                    ...
                    if c1 == 0 {
                        ...
                        if c1 >= 'A' as i32 &&
                            ...
                            c1 =
                                (c1 +
                                     ('a' as i32 - 'A' as i32))                              libc::c_uchar
                        }
                        if c2 >= 'A' as i32 &&
                               c2 <= 'Z' as i32 {
                            c2 =
 ...
 -> i32 {
  ...
  /* j counts no. of labels */
    ...
    if hostname_isequal(&lastchunk, "arpa\x00") && hostname_isequal(penchunk, b"in-addr") {
	 ...
	 and should be set to zero. */
        ...
        while cp1 != penchunk {
	     ...
	     as CNAME targets according to RFC 2317 */
            ...
            cp = cp1;
            while cp != 0 {
                if *(*__ctype_b_loc()).offset(cp                                            libc::c_int) &
            ...
            *addr.offset(0) =
                atoi(cp1);
            cp1 =
                cp1.offset(strlen(cp1).wrapping_add(1     libc::c_ulong)                         isize)
        }
        return ((1) << 7)
    } else {
      ...
      */
            if *name.as_mut_ptr() == '\\' as i32 &&
                   *name.as_mut_ptr().offset(1) == '[' as i32 &&
                   (*name.as_mut_ptr().offset(2)  == 'x' as i32 ||
                        *name.as_mut_ptr().offset(2)
                            == 'X' as i32) {
                j = 0;
                cp1 = name.as_mut_ptr().offset(3);
                while cp1 != 0 &&
                          *(*__ctype_b_loc()).offset(cp1      libc::c_int)
                              &
                              _ISXDIGIT                             libc::c_int != 0 && j < 32 {
                ...
                while cp1 != penchunk {
                    if cp1.offset(1)
                           != 0 ||
                           *(*__ctype_b_loc()).offset(cp1       libc::c_int       isize) &
                               _ISXDIGIT                              libc::c_int == 0 {
                        return 0
                    }
                    j =
                        (::std::mem::size_of::<In6Addr>()).wrapping_sub(1   )                      libc::c_int;
                    while j > 0 {
                        *addr.offset(j) =
                            (*addr.offset(j) >>
                                 4 |
                                 (*addr.offset((j - 1)isize)) <<
                                     4);
                        j -= 1
                    }
                    *addr.offset(0) =
                        ((*addr.offset(0) >> 4)
                             ...
                             strtol(cp1, 0 ,
                                    16) << 4)                      libc::c_uchar;
                    cp1 =
                        cp1.offset(strlen(cp1).wrapping_add(1
                                                                     )
                                      )
                }
                return ((1) << 8)
                ...
                match current_block_28 {
                    ...
                    _ => {
                      ...
                      dnsmasq_daemon.options[(2  libc::c_ulong).wrapping_div((::std::mem::size_of::<libc::c_uint>()
                          ...
                          != 0 {
            let mut p1: mut Vec<u8> = p;
            if !((p1.wrapping_offset_from(header)                i32 + rdlen)  <= qlen)
               {
                return 0
            }
            while (p1.wrapping_offset_from(p)) <
                      rdlen {
                let mut i_0: u32 = 0;
                let mut len: u32 = *p1;
                let mut p2: mut Vec<u8> = p1;
                if p1.offset(len).wrapping_offset_from(p)                 i32 >= rdlen {
                    return 0
                }
                /* make counted string zero-term  and sanitise */
                i_0 = 0;
                while i_0 < len {
                    if *(*__ctype_b_loc()).offset(*p2.offset(1
    ...
    while i != 0 {
        ...
        ttl =
            ((*t_cp_1.offset(0)_0) <<
                 ...
                 (*t_cp_1.offset(2)_0) <<
                     8 |
                 *t_cp_1.offset(3)_0)          libc::c_ulong;
        p = p.offset(4);
        ...
        if qclass == 1 && qtype == 6 {
            ...
            p =
                skip_name(p, header, qlen,
                          20); /* SERIAL REFRESH RETRY EXPIRE */
            if p.is_null() { return 0 }
            p = p.offset(16);
            ...
            /* minTTL */
            if ttl < minttl { minttl = ttl }
        } else if if !((p.wrapping_offset_from(header)
                            + rdlen)
                           <= qlen) {
                      0
                  } else { p = p.offset(rdlen); 1 } ==
                      0 {
            return 0
        }
        i -= 1
...
pub fn extract_addresses(mut header: DnsHeader,
    ...
    while i != 0 {
        ...
        let mut cttl: u32 =
            (9223372036854775807    ).wrapping_mul(2                                    ).wrapping_add(1
                                                                                                       );
        let mut attl: u32 = 0;
        namep = p;
                        ...
                        'c_14031:
                            loop  {
                                ...
                                loop  {
                                                    ...
                                                    0) == 0 ||
                                           {
                                               res =
                                                   extract_name(header, qlen,
                                                                &mut p1, name,
                                                                0                 libc::c_int,
                                                                10                 libc::c_int); /* bad packet */
                                               (res) == 0
                                           } {
                                        return 0
                                    }
                                    let mut t_cp_1: mut Vec<u8> = p1;
                                    aqtype =
                                        (*t_cp_1.offset(0         isize)                                       libc::c_int) << 8
                                            |
                                            *t_cp_1.offset(1            isize)
                                               ;
                                    p1 = p1.offset(2);
                                    let mut t_cp_2: mut Vec<u8> = p1;
                                    aqclass =
                                        (*t_cp_2.offset(0         isize)                                       libc::c_int) << 8
                                            |
                                            *t_cp_2.offset(1            isize)
                                               ;
                                    p1 = p1.offset(2);
                                    let mut t_cp_3: mut Vec<u8> = p1;
                                    attl =
                                        ((*t_cp_3.offset(0          isize)_0)
                                             << 24 |
                                             (*t_cp_3.offset(1
                                                                )                                            u32_0) << 16
                                             |
                                    ...
                                    let mut t_cp_5: mut Vec<u8> = p1;
                                    ardlen =
                                        (*t_cp_5.offset(0         isize)                                       libc::c_int) << 8
                                            |
                                            *t_cp_5.offset(1            isize)
                                               ;
                                    p1 = p1.offset(2);
                                    ...
                                    p1 = endrr;
                                    if !((p1.wrapping_offset_from(header                   mut Vec<u8>)
                                              +
                                              0                                            i32)  <=
                                             qlen) {
                                        return 0
                        ...
                        'c_10467:
                            loop  {
                                ...
                                loop  {
                                    ...
                                    let mut t_cp_6: mut Vec<u8> =
                                        p1; /* bad packet */
                                    aqtype =
                                        (*t_cp_6.offset(0         isize)                                       libc::c_int) << 8
                                            |
                                            *t_cp_6.offset(1            isize)
                                                                                         libc::c_int; /* include terminating zero */
                                    p1 = p1.offset(2);
                                    let mut t_cp_7: mut Vec<u8> = p1;
                                    aqclass =
                                        (*t_cp_7.offset(0         isize)                                       libc::c_int) << 8
                                            |
                                            *t_cp_7.offset(1            isize)
                                               ;
                                    p1 = p1.offset(2);
                                    let mut t_cp_8: mut Vec<u8> = p1;
                                    attl =
                                        ((*t_cp_8.offset(0          isize)_0)
                                             << 24 |
                                             (*t_cp_8.offset(1
                                                                )                                            u32_0) << 16
                                             |
                                    ...
                                    if dnsmasq_daemon.max_ttl !=
                                        ...
                                        *fresh19 =
                                            (t_l_0 >> 24)                                          libc::c_uchar;
                                        let fresh20 = t_cp_9;
                                        t_cp_9 = t_cp_9.offset(1);
                                        *fresh20 =
                                            (t_l_0 >> 16)                                          libc::c_uchar;
                                        let fresh21 = t_cp_9;
                                        t_cp_9 = t_cp_9.offset(1);
                                        *fresh21 =
                                            (t_l_0 >> 8)                                          libc::c_uchar;
                                        *t_cp_9 = t_l_0;
                                        p1 =
                                            p1.offset(4       isize)
                                    }
                                    let mut t_cp_10: mut Vec<u8> = p1;
                                    ardlen =
                                        (*t_cp_10.offset(0          isize)
                                            ) <<
                                            8 |
                                            *t_cp_10.offset(1
                                                               )                                          u16;
                                            ...
                                            if flags &
                                                       ...
                                                       30 != 0
                                               {
                                                ...
                                                let mut t_cp_11:
                                                    ...
                                                    p1;
                                                addr.srv.priority =
                                                    ((*t_cp_11.offset(0                       libc::c_int
                                                          )
                                                                libc::c_int) <<
                                                         8 |
                                                         *t_cp_11.offset(1                          libc::c_int
                                                                )
                                                                      libc::c_int)     u16;
                                                p1 =
                                                    p1.offset(2
                                                ...
                                                let mut t_cp_12:
                                                    ...
                                                    p1;
                                                addr.srv.weight =
                                                    ((*t_cp_12.offset(0                       libc::c_int
                                                          )
                                                                libc::c_int) <<
                                                         8 |
                                                         *t_cp_12.offset(1                          libc::c_int
                                                                )
                                                                      libc::c_int)     u16;
                                                p1 =
                                                    p1.offset(2
                                                ...
                                                let mut t_cp_13:
                                                    ...
                                                    p1;
                                                addr.srv.srvport =
                                                    ((*t_cp_13.offset(0                       libc::c_int
                                                          )
                                                                libc::c_int) <<
                                                         8 |
                                                         *t_cp_13.offset(1                          libc::c_int
                                                                )
                                                                      libc::c_int)     u16;
                                                p1 =
                                                    p1.offset(2
                ...
                if !(!limit.is_null() &&
                    ...
                    *t_cp_4 = t_s_3;
                    ...
                    loop 
                        ...
                        match *format {
                            ...
                            100 => {
                                ...
                                if !offset.is_null() {
                                    *offset =
                                        p.wrapping_offset_from(header                mut Vec<u8>)
                    ...
                    match current_block {
                        ...
                        {
                            j =
                                (p.wrapping_offset_from(sav) -
                                     2)                              libc::c_int;
                            let mut t_s_5: u16 = j;
                            let mut t_cp_7: mut Vec<u8> = sav;
                            let fresh40 = t_cp_7;
                            t_cp_7 = t_cp_7.offset(1);
                            *fresh40 =
                                (t_s_5 >> 8)                              libc::c_uchar;
                            *t_cp_7 = t_s_5;
                            sav = sav.offset(2);
                            *pp = p;
                            return 1
...
pub fn answer_request(mut header: DnsHeader,
    ...
    loop 
        ...
        while q != 0 {
            ...
            nameoffset =
                p.wrapping_offset_from(header)              i32;
            /* now extract name as .-concatenated string into name */
            if extract_name(header, qlen, &mut p, name, 1,
                            4) == 0 {
                return 0
            } /* bad packet */
            let mut t_cp: mut Vec<u8> =
            ...
            loop  {
                ...
                let mut cname_target: &mut String =
            ...
            if qclass == 1 {
                ...
                if qtype == 12 ||
                    ...
                    if !intr.is_null() {
                        ...
                        crecp =
                            cache_find_by_addr(0 , &mut addr, now,
                                               is_arpa);
                        if !crecp.is_null() {
		     ...
		     the query, is so we'll filter other answers. */
                                if local_addr.s_addr !=
                                       ...
                                       dnsmasq_daemon.options[(18                   libc::c_int
                                           ...
                                           &
                                           (1) <<
                                               (18 libc::c_ulong).wrapping_rem((::std::mem::size_of::<libc::c_uint>()
                                                                                                               ).wrapping_mul(8                                                         libc::c_int                                                  ))
                                           != 0 &&
                                       flag ==
                                           (1) <<
			       ...
			       filter here. */
                    ...
                    if found_0 == 0 &&
                           dnsmasq_daemon.options[(1).wrapping_div((::std::mem::size_of::<libc::c_uint>()
                               ...
                               (1) <<
                                   (1 ).wrapping_rem((::std::mem::size_of::<libc::c_uint>()
                                                                                       ).wrapping_mul(8))
                               != 0 &&
                           (qtype == 33 ||
                                qtype == 255 &&
