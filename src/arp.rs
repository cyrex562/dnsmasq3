...
static mut freelist: *mut arp_record =
    0 as *const arp_record as *mut arp_record;
...
unsafe extern "C" fn filter_mac(mut family: libc::c_int,
    ...
    if maclen > 16 as libc::c_int as libc::c_ulong { return 1 as libc::c_int }
    let mut current_block_8: u64;
    /* Look for existing entry */
    arp = arps;
    while !arp.is_null() {
        if !(family != (*arp).family ||
                 (*arp).status as libc::c_int == 2 as libc::c_int) {
