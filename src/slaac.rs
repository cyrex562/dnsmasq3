...
pub mod uintn_identity_h {
    ...
    use super::types_h::{__uint16_t, __uint64_t, __uint32_t};
}
...
pub mod if_arp_h {
    ...
    #[c2rust::src_loc = "78:9"]
    pub const ARPHRD_IEEE802: libc::c_int = 6 as libc::c_int;
    #[c2rust::src_loc = "73:9"]
...
pub mod asm_generic_errno_h {
    #[c2rust::src_loc = "96:9"]
    pub const EHOSTUNREACH: libc::c_int = 113 as libc::c_int;
}
...
pub mod icmp6_h {
    #[c2rust::src_loc = "67:9"]
    pub const ICMP6_ECHO_REQUEST: libc::c_int = 128 as libc::c_int;
}
...
pub use self::socket_h::{socklen_t, sockaddr, msghdr, cmsghdr, PF_INET6,
                         AF_INET6, __cmsg_nxthdr};
...
pub use self::in_h::{sockaddr_in6, in6_addr, C2RustUnnamed, in_port_t,
                     ...
                     IPPROTO_HOPOPTS, INET6_ADDRSTRLEN};
...
pub use self::dnsmasq_h::{u8_0, u16_0, u32_0, all_addr, C2RustUnnamed_1,
                          ...
                          dhcp_relay, dnsmasq_daemon, CONTEXT_OLD,
                          CONTEXT_RA_NAME, LEASE_NA, LEASE_TA,
                          LEASE_HAVE_HWADDR, MS_DHCP, ADDRSTRLEN, rand16,
                          whine_malloc, my_syslog, lease_update_dns,
                          reset_counter, save_counter, expand,
                          ra_start_unsolicited};
pub use self::radv_protocol_h::ping_packet;
...
pub use self::uintn_identity_h::{__uint16_identity, __uint64_identity,
                                 __uint32_identity};
...
pub use self::if_arp_h::{ARPHRD_IEEE1394, ARPHRD_EUI64, ARPHRD_IEEE802,
                         ARPHRD_ETHER};
pub use self::asm_generic_errno_h::EHOSTUNREACH;
pub use self::syslog_h::{LOG_INFO, LOG_DAEMON};
pub use self::icmp6_h::ICMP6_ECHO_REQUEST;
...
pub unsafe extern "C" fn slaac_add_addrs(mut lease: *mut dhcp_lease,
    ...
    let mut old = 0 as *mut slaac_address;
    let mut up = 0 as *mut *mut slaac_address;
    let mut context = 0 as *mut dhcp_context;
    let mut dns_dirty = 0 as libc::c_int;
    ...
    (*lease).slaac_address = NULL_0 as *mut slaac_address;
...
pub unsafe extern "C" fn periodic_slaac(mut now: time_t,
 ...
 -> time_t {
    ...
    let mut slaac = 0 as *mut slaac_address;
    let mut next_event = 0 as libc::c_int as time_t;
    context = (*dnsmasq_daemon).dhcp6;
    while !context.is_null() {
        if (*context).flags as libc::c_uint & CONTEXT_RA_NAME != 0 &&
               (*context).flags as libc::c_uint & CONTEXT_OLD == 0 {
        ...
        while !slaac.is_null() {
            ...
            if !((*slaac).backoff == 0 as libc::c_int ||
                ...
                if difftime((*slaac).ping_time, now) <= 0.0f64 {
                    let mut ping = 0 as *mut ping_packet; /* Give up */
                    let mut addr =
                    ...
                    if ping.is_null() {
                        ...
                        (*ping).type_0 = ICMP6_ECHO_REQUEST as u8_0;
...
pub unsafe extern "C" fn slaac_ping_reply(mut sender: *mut in6_addr,
    ...
    let mut lease = 0 as *mut dhcp_lease;
    let mut slaac = 0 as *mut slaac_address;
    let mut ping = packet as *mut ping_packet;
    ...
    if (*ping).identifier as libc::c_int == ping_id {
        ...
        while !lease.is_null() {
            ...
            while !slaac.is_null() {
                if (*slaac).backoff != 0 as libc::c_int &&
                       ({
                            let mut __a = sender as *const in6_addr;
                            let mut __b =
                                &mut (*slaac).addr as *mut in6_addr as
                                    *const in6_addr;
                    ...
                    inet_ntop(AF_INET6, sender as *const libc::c_void,
